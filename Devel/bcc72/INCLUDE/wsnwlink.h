...
typedef struct _IPX_NETNUM_DATA {
    ...
    INT    cardnum;    /* output: 0-based adapter number used to route to this net; */ /* __CODEGEARC__ fix for nested comments */
                       /*         can be used as adapternum input to IPX_ADDRESS */
    UCHAR  router[6];  /* output: MAC address of the next hop router, zeroed if */     /* __CODEGEARC__ fix for nested comments */
