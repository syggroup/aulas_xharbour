...
typedef interface IDCompositionFloodEffect               IDCompositionFloodEffect;
...
STDAPI DCompositionGetFrameStatistics(
    ...
    _Out_opt_ D3D_FEATURE_LEVEL *minFeaturelLevel);
...
DECLARE_INTERFACE_IID_(IDCompositionDevice3, IDCompositionDevice2, "0987CB06-F916-48BF-8D35-CE7641781BD9")
{
    // Creates a new visual object.
    STDMETHOD(CreateVisual)(THIS_
        _Outptr_ IDCompositionVisual3 **visual
        ) PURE;
...
DECLARE_INTERFACE_IID_(IDCompositionVisual3, IDCompositionVisualDebug, "D7715AE9-7B40-44FA-A1A8-51C559E4C4FA")
{
    // Sets transform mode property associated with this visual
    STDMETHOD(SetTransformMode)(THIS_
        _In_ DCOMPOSITION_TRANSFORM_MODE mode
        ) PURE;

    // Sets the 4x4 matrix that modifies the coordinate system of this visual.
    STDMETHOD(SetTransform)(THIS_
        const D2D_MATRIX_4X4_F& matrix
        ) PURE;

    // Sets the 4x4 transform
    STDMETHOD(SetTransform)(THIS_
...
//
//  Synopsis:
//      An IDCompositionFloodEffect interface represents a flood filter effect
//
//------------------------------------------------------------------------------
#undef INTERFACE
