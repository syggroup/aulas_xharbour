...
void          _RTLENTRY _EXPFUNC _NO_RETURN_DECL exit(int __status);
...
char *        _RTLENTRY _EXPFUNC _ecvt(double __value, int __ndig, int * __dec,
                                     int * __sign);
void          _RTLENTRY _EXPFUNC _NO_RETURN_DECL _exit(int __status);
...
/*
  ...
  Standard Library need to #undef them.
...
#if defined(__MFC_COMPAT__) && !defined(__STDC__)
     using std::_itoa;
     using std::_ultoa;
     using std::_putenv;
     using std::_swab;
