...
//       Version 0x0600: C++ Builder 6.0
//       Version 0x0700: C++ Builder 2006
///////////////////////////////////////////////////////////////////////////////

#define __UTILCLS_H_VERSION 0x0800
...
template <class T>
int DebugHlpr_PROMPT(const T* caption, const T* msg);
...
#else
void DebugHlpr_THROW(T* msg, HRESULT, T*, bool /*assertFailed*/)
{
...
HRESULT DebugHlpr_HRCHECK(HRESULT hr, T* expr, T* file, int line)
{
  if (!SUCCEEDED(hr))
  {
    ...
    int i = IDYES;
#else
    int i = DebugHlpr_PROMPT<T>(_T("HRCHECK: "), szMsg);
#endif
    if (i == IDYES)
...
template <class T>
int DebugHlpr_ASSERTE(const T* expr, const T* file, int line)
{
  TCHAR msg[_MAX_PATH*2];
  TCHAR lfile[_MAX_PATH*2];
  ::wsprintf(msg, (sizeof(TCHAR) == sizeof(T)) ?
                  _T("%s @ %s/%d") : _T("%S @ %S/%d"),
                  LPTSTR(expr), file, line);
  ...
  {
    ::wsprintf(msg, (sizeof(TCHAR) == sizeof(T)) ?
                    _T("%s failed - %s/%d") : _T("%S failed - %S/%d"),
                    expr, file, line);
    ::wsprintf(lfile, (sizeof(TCHAR) == sizeof(T)) ? _T("%s") : _T("%S"), file);
...
int DebugHlpr_PROMPT(const T* caption, const T* msg)
{
  TCHAR szMsg[_MAX_PATH*2];
  ::wsprintf(szMsg, (sizeof(TCHAR) == sizeof(T))
                ? _T("%s\nPress [Y]es to terminate,"
                     "[N]o to continue and [C]ancel to Debug")
                : _T("%S\nPress [Y]es to terminate,"
                     "[N]o to continue and [C]ancel to Debug"), msg);
  return ::MessageBox(0, szMsg, caption, MB_TASKMODAL|MB_ICONQUESTION|MB_YESNOCANCEL);
}

#ifndef UTILCLS_NO_SAFECALL_ERROR_INFO
// The following specialization handles safecall exceptions and
// generates an EOleException with rich error info.
// Implemented in comhlpr.cpp
template <>
HRESULT DebugHlpr_HRCHECK(HRESULT hr, const char* expr,
                          const char* file, int line);
#endif // UTILCLS_NO_SAFECALL_ERROR_INFO
...
#if !defined(OLECHECK_QI)
#define OLECHECK_QI(hrexpr) ((void)0)
#endif
...
void __cdecl TDebugHlpr<T>::TRACE_HLPR(T* szFormat, ...)
{
  ...
  _ASSERTE(bufSize < (sizeof(szBuffer)/sizeof(szBuffer[0])));
...
// For historical reason Utilscls.h is not namespace aware.
// However, now TStringConverter conflicts with "Data::Dbxjsonreflect::TStringConverter"
// Ultimately "Utilcls.h" should be namespaced. But for now we'll handle just the
// "TStringConverter" issue.
namespace Utilcls {
...
public:
  ...
  static  GUID      AnsiToGUID(LPCSTR     src);
...
template <class T> GUID
TStringConverter<T>::AnsiToGUID(LPCSTR src)
{
  GUID retval;
...
TStringConverter<T>::AnsiToOLESTR(LPCSTR src)
{
   ...
   /* MultiByteToWideChar includes terminating null but SysAllocStringLen
      allocates it automatically - so adjust */
   LPWSTR dst = ::SysAllocStringLen(0, size-1);
...
{
  return Utilcls::TStringConverter<TCHAR>::WideToAnsi(src);
}

// NOTE: Return value must be passed to '::SysFreeString'
inline OLECHAR* AnsiToOLESTR(LPCSTR src)
{
  return Utilcls::TStringConverter<TCHAR>::AnsiToOLESTR(src);
}

// NOTE: Return value must be '::SysFreeString'
inline OLECHAR* WideToOLESTR(LPCWSTR src)
{
  return Utilcls::TStringConverter<TCHAR>::WideToOLESTR(src);
}
...
{
   _ASSERTE_(str != 0);
   return Utilcls::TStringConverter<TCHAR>::strnewdup(str);
}
...
{
public:
  explicit TOleString(BSTR bstr, boolean alloc=false) : m_bstr(alloc ? ::SysAllocString(bstr) : bstr) {}
...
public:
  ...
  operator bool() const { return m_Proc != 0; }
protected:
  FARPROC   m_Proc;
};

////////////////////////////////////////////////////////////////
// cdecl FARPROC - 1 Param
////////////////////////////////////////////////////////////////
template <class P1, class R>
class TDllProc1 : public TDllProc
{
public:
  TDllProc1(const TDll& dll , LPCSTR funcName) : TDllProc(dll, funcName) {};
  R operator ()(P1 erste)
  {
    typedef R (__cdecl* outproc)(P1 erste);
    return ((outproc)m_Proc)(erste);
  }
};

//////////////////////////////////////////////////////////////////
// stdcall FARPROC - 1 Param
//////////////////////////////////////////////////////////////////
template <class P1, class R = HRESULT>
class TDllStdProc1 : public TDllProc
{
public:
  TDllStdProc1(const TDll& dll, LPCSTR funcName) : TDllProc(dll, funcName) {}
  R operator () (P1 p1)
  {
    typedef R (__stdcall* outproc)(P1 p1);
    return ((outproc)m_Proc)(p1);
  }
};

//////////////////////////////////////////////////////////////////
// stdcall FARPROC - 2 Params
//////////////////////////////////////////////////////////////////
template <class P1, class P2, class R = HRESULT>
class TDllStdProc2 : public TDllProc
{
public:
  TDllStdProc2(const TDll& dll, LPCSTR funcName) : TDllProc(dll, funcName) {}
  R operator () (P1 p1, P2 p2)
  {
    typedef R (__stdcall* outproc)(P1 p1, P2 p2);
    return ((outproc)m_Proc)(p1, p2);
  }
};

//////////////////////////////////////////////////////////////////
// cdecl FARPROC - 2 Parameters
//////////////////////////////////////////////////////////////////
template <class P1, class P2, class R>
class TDllProc2 : public TDllProc
{
public:
  TDllProc2(const TDll& dll, LPCSTR funcName) : TDllProc(dll, funcName) {}
  R operator () (P1 p1, P2 p2)
  {
    typedef R (__cdecl* outproc)(P1 p1, P2 p2);
    return ((outproc)m_Proc)(p1, p2);
  }
};

//////////////////////////////////////////////////////////////////
// stdcall FARPROC - 3 Params
//////////////////////////////////////////////////////////////////
template <class P1, class P2, class P3, class R = HRESULT>
class TDllStdProc3 : public TDllProc
{
public:
  TDllStdProc3(const TDll& dll, LPCSTR funcName) : TDllProc(dll, funcName) {}
  R operator () (P1 p1, P2 p2, P3 p3)
  {
    typedef R (__stdcall* outproc)(P1 p1, P2 p2, P3 p3);
    return ((outproc)m_Proc)(p1, p2, p3);
  }
};
...
public:
  ...
  bool operator==(bool val) const   { return val ? (m_VariantBool != VARIANT_FALSE) : (m_VariantBool == VARIANT_FALSE); }
  bool operator==(VARIANT_BOOL val) const { return m_VariantBool == val; }
...
{
public:
  static bool IsEqual(V& v, TYPE t);
};

template <typename V, typename TYPE>
bool VariantCompare<V, TYPE>::IsEqual(V& v, TYPE t)
{
  V tmp(t);
  if (v.vt != tmp.vt)
    return false;
  switch(v.vt)
  {
    ...
    case VT_BOOL:
      return V_BOOL(&v) == V_BOOL(&tmp);

    case VT_UI1:
      return V_UI1(&v) == V_UI1(&tmp);

    case VT_I2:
      return V_I2(&v) == V_I2(&tmp);

    case VT_I4:
      return V_I4(&v) == V_I4(&tmp);

    case VT_I8:
      return V_I8(&v) == V_I8(&tmp);

    case VT_UI8:
      return V_UI8(&v) == V_UI8(&tmp);

    case VT_R4:
      return V_R4(&v) == V_R4(&tmp);

    case VT_R8:
      return V_R8(&v) == V_R8(&tmp);

    case VT_CY:
      return memcmp(&(V_CY(&v)), &(V_CY(&tmp)), sizeof(CY)) == 0;

    case VT_DATE:
      return V_DATE(&v) == V_DATE(&tmp);

    case VT_BSTR:
      return (::SysStringByteLen(V_BSTR(&v)) == ::SysStringByteLen(V_BSTR(&tmp))) &&
          (memcmp(V_BSTR(&v), V_BSTR(&tmp), ::SysStringByteLen(V_BSTR(&v))) == 0);

    case VT_DISPATCH:
      return V_DISPATCH(&v) == V_DISPATCH(&tmp);

    case VT_ERROR:
      return V_ERROR(&v) == V_ERROR(&tmp);

    case VT_UNKNOWN:
      return V_UNKNOWN(&v) == V_UNKNOWN(&tmp);

    case VT_DECIMAL:
      return memcmp(&(V_DECIMAL(&v)), &(V_DECIMAL(&tmp)), sizeof(DECIMAL)) == 0;
  }
  return false;
}
}
...
#define SET_VTYPE_AND_VAR(type, val)    { this->vt = VT_ ## type ;           \
                                          V_ ## type (this) = val; }
#endif
#if !defined(SET_VTYPE_AND_VARREF)
#define SET_VTYPE_AND_VARREF(type, val) { this->vt = VT_ ## type | VT_BYREF; \
...
public:
 ...
 ~TVariantT()
  ...
  void __fastcall swap (TVariantT& rhs)
  {
    char temp[sizeof(TVariantT)];
    std::memcpy(temp, this, sizeof(TVariantT));
    std::memcpy(this, &rhs, sizeof(TVariantT));
    std::memcpy(&rhs, temp, sizeof(TVariantT));
  }

  TVariantT(const TVariantT& src)
  {
    ::VariantInit(this);
    OLECHECK(::VariantCopy(this, (VARIANT*)&src));
  }
  ...
  TVariantT(__int64 src)
  {
    ::VariantInit(this);
    SET_VTYPE_AND_VAR(I8, src);
  }
  TVariantT(unsigned __int64 src)
  {
    ::VariantInit(this);
    SET_VTYPE_AND_VAR(UI8, src);
  }
  ...
  TVariantT(const VARIANT& src, bool copy = true)
  {
    ...
    if (copy)
    {
      OLECHECK(::VariantCopy(this, const_cast<VARIANTARG*>(&src)));
    }
    else
    {
  ...
  TVariantT(const System::Variant& rhs)
  {
    ...
    OLECHECK(::VariantCopy(this, reinterpret_cast<VARIANT*>(&v)));
  }
#endif
#if defined(DSTRING_H)
  TVariantT(const System::AnsiString& src)
  {
    ::VariantInit(this);
    *this = src.c_str();
  }
  ...
  TVariantT(const System::Currency& src)
  {
    ::VariantInit(this);
    SET_VTYPE_AND_VAR(CY, *(reinterpret_cast<tagCY*>(&(const_cast<System::Currency&>(src).Val))))
  }
  TVariantT(System::Currency* src)
  {
    ::VariantInit(this);
  ...
  TVariantT(const System::TDateTime& src)
  {
    ::VariantInit(this);
    SET_VTYPE_AND_VAR(DATE, src);
  }
  TVariantT(System::TDateTime* src)
  {
    ::VariantInit(this);
    SET_VTYPE_AND_VARREF(DATE, reinterpret_cast<DATE*>(src));
  }
#endif
#if defined(WSTRING_H) && !defined(_DELPHI_NEXTGEN)
  TVariantT(const System::WideString& src)
  {
    ::VariantInit(this);
    SET_VTYPE_AND_VAR(BSTR, src.Copy());
  }
#endif
#if defined(USTRING_H) && !defined(_DELPHI_NEXTGEN)
  TVariantT(const System::UnicodeString& src)
  {
    ::VariantInit(this);
    SET_VTYPE_AND_VAR(BSTR, System::WideString(src).Copy());
  }
#endif
  ...
  // override.
  // NOTE: The 'isSysString' flag is really to determine whether
  // TVariant should make a copy of the BSTR passed in. In cases
  // where 'src' is not a BSTR, then a copy should be made.
  // However, it's also valid to have TVariant make a copy in cases
  // where you want to maintain ownership of the BSTR passed in.
  ...
  // NOTE: 'DATE' is just a 'double' - hence
  ...
  TVariantT& operator=(__int64 src)
  {
    Clear();
    SET_VTYPE_AND_VAR(I8, src);
    return *this;
  }
  TVariantT& operator=(unsigned __int64 src)
  {
    Clear();
    SET_VTYPE_AND_VAR(UI8, src);
    return *this;
  ...
  // NOTE: 'DATE' and 'double' are identical.
  //       Hence there's no way to distinguish
  //       However, most implementation can handle
  //       either form...
  ...
  TVariantT& operator =(SAFEARRAY* rhs)
  {
    TVariantT newVal(rhs);
    swap(newVal);
    return *this;
  ...
  {
    TVariantT newVal(reinterpret_cast<VARIANT*>(&(const_cast<System::OleVariant&>(rhs))));
    swap(newVal);
    return *this;
  }
...
#endif
#if defined(DSTRING_H)
  TVariantT& operator=(const System::AnsiString& rhs)
  {
    TVariantT newVal(rhs);
    swap(newVal);
    return *this;
...
#endif
#if defined(SYSCURR_H)
  TVariantT& operator=(const System::Currency& src)
  {
    Clear();
    SET_VTYPE_AND_VAR(CY, *(reinterpret_cast<tagCY*>(&(const_cast<System::Currency&>(src).Val))))
    return *this;
...
#endif
#if defined(SYSTDATE_H)
  TVariantT& operator=(const System::TDateTime& src)
  {
    Clear();
    SET_VTYPE_AND_VAR(DATE, src);
    return *this;
  ...
  TVariantT& operator=(const System::WideString& src)
  {
    TVariantT newVal(src);
    swap(newVal);
    return *this;
  }
#endif
#if defined(USTRING_H)
  TVariantT& operator=(const System::UnicodeString& src)
  {
    TVariantT newVal(src);
    swap(newVal);
    return *this;
  ...
  {
    ::VariantClear(this);
    TVariantT newVal(src, false/*Make BSTR copy*/);
    swap(newVal);
  }
...
#endif
#if defined(SYSTDATE_H)
  TVariantT& operator=( System::TDateTime* src)
  {
    Clear();
  ...
  {
    return OLECHECK(::VariantChangeType(this, const_cast<VARIANT*>(&src), 0, vtNew));
  }
  HRESULT   ChangeType(VARTYPE vtNew)
  {
    return OLECHECK(::VariantChangeType(this, this, 0, vtNew));
  }
  HRESULT    Clear()
  {
    return OLECHECK(::VariantClear(this));
  }
  ...
  {
    TVariantT<T, true> v(*this);
    HRESULT hr = v.ChangeType(VT_UI1);
    OLECHECK(hr);
    return V_UI1(&v);
  }
  ...
  {
    TVariantT<T, true> v(*this);
    HRESULT hr = v.ChangeType(VT_UI2);
    OLECHECK(hr);
    return V_UI2(&v);
  }
  operator int() const
  ...
  {
    TVariantT<T, true> v(*this);
    HRESULT hr = v.ChangeType(VT_UI4);
    OLECHECK(hr);
    return V_UI4(&v);
  }
  operator long() const
  ...
  {
    TVariantT<T, true> v(*this);
    HRESULT hr = v.ChangeType(VT_UI8);
    OLECHECK(hr);
    return V_UI8(&v);
  }
#if defined(DSTRING_H)
...
#endif
#if defined(USTRING_H)
  operator System::UnicodeString() const;
#endif
#if defined(SYSCURR_H)
  operator System::Currency() const
  {
    TVariantT<T, true> v(*this);
    HRESULT hr = v.ChangeType(VT_CY);
    OLECHECK(hr);
    System::Currency retval;
    retval.Val = V_CY(&v).int64;
    return retval;
  }
#endif
#if defined(SYSTDATE_H)
  operator System::TDateTime() const
  {
    TVariantT<T, true> v(*this);
    HRESULT hr = v.ChangeType(VT_DATE);
    OLECHECK(hr);
    return System::TDateTime(V_DATE(&v));
  }
#endif
  ...
  // NOTE: The data returns is still owned
  ...
  {
    if (this->vt == (VT_I1|VT_BYREF))
      return (signed char*)V_I1REF(this);
    else if (this->vt == VT_I1)
      return (signed char*)&(V_I1(this));
    return NULL;
  }
  operator unsigned char*()
  {
    if (this->vt == (VT_UI1 | VT_BYREF))
  ...
  operator short*()
  {
    if (this->vt == (VT_I2|VT_BYREF))
      return V_I2REF(this);
    else if (this->vt == VT_I2)
      return &(V_I2(this));
    return NULL;
  }
  operator unsigned short*()
  {
    if (this->vt == (VT_UI2|VT_BYREF))
  ...
  {
    if (this->vt == (VT_UI4|VT_BYREF))
      return (unsigned int*)V_UI4REF(this);
    else if (this->vt == VT_UI4)
  ...
  operator long*()
  {
    _ASSERTE(this->vt!= VT_UNKNOWN);
    _ASSERTE(this->vt!= VT_DISPATCH);
    _ASSERTE(this->vt!= (VT_UNKNOWN|VT_BYREF));
    _ASSERTE(this->vt!= (VT_DISPATCH|VT_BYREF));

    if (this->vt == (VT_UI4|VT_BYREF))
      return (long*)V_UI4REF(this);
    else if (this->vt == VT_UI4)
  ...
  operator float*()
  {
    if (this->vt == (VT_R4|VT_BYREF))
      return V_R4REF(this);
    else if (this->vt == VT_R4)
      return &(V_R4(this));
    return NULL;
  }
  operator double*()
  {
    if (this->vt == (VT_R8|VT_BYREF))
      return V_R8REF(this);
    else if (this->vt == VT_R8)
      return &(V_R8(this));
    return NULL;
  }
  operator __int64*()
  {
    if (this->vt == (VT_I8|VT_BYREF))
      return (__int64*)V_I8REF(this);
    else if (this->vt == VT_I8)
      return (__int64*)&(V_I8(this));
    return NULL;
  }
  operator unsigned __int64*()
  {
    if (this->vt == (VT_UI8|VT_BYREF))
      return (unsigned __int64*)V_UI8REF(this);
    else if (this->vt == VT_UI8)
      return (unsigned __int64*)&(V_UI8(this));
    return NULL;
  }
  operator wchar_t**()
  {
...
#if defined(SYSCURR_H)
  operator System::Currency*()
  {
    if (this->vt==(VT_CY|VT_BYREF))
      return reinterpret_cast<System::Currency*>(this->pcyVal);
    else if (this->vt==VT_CY)
      return reinterpret_cast<System::Currency*>(&(this->cyVal));
    return NULL;
  }
#endif
#if defined(SYSTDATE_H)
  operator System::TDateTime*()
  {
    if (this->vt==(VT_DATE|VT_BYREF))
      return reinterpret_cast<System::TDateTime*>(this->pdate);
    else if (this->vt==VT_DATE)
      return reinterpret_cast<System::TDateTime*>(&(this->date));
    return NULL;
  }
#endif
  ...
  {
    return Utilcls::VariantCompare<T, TYPE>::IsEqual(*this, rhs);
  }
...
// To extract an IUnknown* or IDispatch* from a Variant.
// Moved here so code can be shared between System::Variant & TVariantT
template <typename V>
class VariantIntfExtract
{
public:
  static IDispatch* GetIDispatch(V& v);
  static IUnknown*  GetIUnknown(V& v);
};

template <typename V>
IDispatch* VariantIntfExtract<V>::GetIDispatch(V& v)
{
  // Handle easy case right away
  //
  int vt = v.vt;
#if defined(_Windows)
  _ASSERTE(System::varNull == VT_NULL);
  _ASSERTE(System::varEmpty== VT_EMPTY);
  _ASSERTE(System::varDispatch == VT_DISPATCH);
  _ASSERTE(System::varUnknown == VT_UNKNOWN);
#endif
  if (vt==System::varNull || vt==System::varEmpty)
    return 0;
  ...
  switch(vt)
  {
    case System::varDispatch:
    case System::varDispatch|System::varByRef:
      disp = (vt==VT_DISPATCH ? V_DISPATCH(&v) : (*(V_DISPATCHREF(&v))));
      if (disp)
        disp->AddRef();
      return disp;

    case System::varUnknown:
    case System::varUnknown|System::varByRef: {
      LPUNKNOWN punk = (vt==VT_UNKNOWN ? V_UNKNOWN(&v) : (*(V_UNKNOWNREF(&v))));
      if (punk) {
    ...
    default:
      TVariantT<VARIANT, true> tmp(v);
      HRESULT hr = tmp.ChangeType(VT_DISPATCH);
      OLECHECK(hr);
      if (V_DISPATCH(&tmp)) {
        V_DISPATCH(&v)->AddRef();
      }
      return V_DISPATCH(&tmp);
  }
#endif
}

template <typename V>
IUnknown* VariantIntfExtract<V>::GetIUnknown(V& v)
{
#if defined(_Windows)
  _ASSERTE(System::varNull == VT_NULL);
  _ASSERTE(System::varEmpty== VT_EMPTY);
  ...
  switch(vt)
  {
    ...
    case VT_UNKNOWN|VT_BYREF:
      punk = (vt==VT_UNKNOWN ? V_UNKNOWN(&v) : (*(V_UNKNOWNREF(&v))));
      if (punk) {
        punk->AddRef();
      }
      return punk;
    ...
    case VT_DISPATCH|VT_BYREF: {
      LPDISPATCH disp = (vt==VT_DISPATCH ? V_DISPATCH(&v) : (*(V_DISPATCHREF(&v))));
      if (disp)
    ...
    default:
      TVariantT<VARIANT, true> tmp(v);
      HRESULT hr = tmp.ChangeType(VT_UNKNOWN);
      OLECHECK(hr);
      if (V_UNKNOWN(&tmp)) {
        V_UNKNOWN(&tmp)->AddRef();
      }
      return V_UNKNOWN(&tmp);
  }
#endif
}
...
{
  return VariantIntfExtract<T>::GetIUnknown(*this);
}
...
{
  return VariantIntfExtract<T>::GetIDispatch(*this);
}

#if defined(DSTRING_H)
template <class T, bool CLEANUP>
TVariantT<T, CLEANUP>::operator System::AnsiString() const
{
  TVariant v(*this);
  HRESULT hr = v.ChangeType(VT_BSTR);
  OLECHECK(hr);
  return System::AnsiString(V_BSTR(&v));
}
#endif
#if defined(WSTRING_H) && !defined(_DELPHI_NEXTGEN)
template <class T, bool CLEANUP>
TVariantT<T, CLEANUP>::operator System::WideString() const
{
  TVariant v(*this);
  HRESULT hr = v.ChangeType(VT_BSTR);
  OLECHECK(hr);
  ...
  //       we own!
  return System::WideString(V_BSTR(&v));
}
#endif
#if defined(USTRING_H)
template <class T, bool CLEANUP>
TVariantT<T, CLEANUP>::operator System::UnicodeString() const
{
  TVariant v(*this);
  HRESULT hr = v.ChangeType(VT_BSTR);
  OLECHECK(hr);
  return System::UnicodeString(V_BSTR(&v));
}
#endif
...
typedef TVariantT<VARIANT, false> TVariantNoClear;
...
public:
  ...
  // Specify whether the interface should be AddRef'ed or not.
  // For an interface obtained via QueryInterface or CoCreateInstance,
  // you don't want to AddRef.
  TComInterface(T* p, bool addRef
#if defined(__clang__)  // FIXME (Workaround for Office binding)
                                 = false
#endif
                                 )
  {
  ...
  TComInterface(const TComInterface<ANOTHERINTF, ANOTHERIID>& src) : intf(0)
  {
    ...
    if (src)
    {
      HRESULT hr = src->QueryInterface(GetIID(), (LPVOID*)(&intf));
      OLECHECK_QI(hr);
    }
  }
 ...
 ~TComInterface()
  ...
  template <class I>
  TComInterface(const System::DelphiInterface<I>& src) : intf(0)
  {
    if (src) {
      *this = src;
    }
  ...
  template <class I>
  TComInterface<T, piid>& operator=(const System::DelphiInterface<I>& src)
  {
    _ASSERTE_(/* Need have valid IID to invoke this */GetIID() != GUID_NULL);
    Reset();
    if (src)
...
TComInterface<T, piid>::operator=(IUnknown* p)
{
  ...
  if (p)
  {
    HRESULT hr = p->QueryInterface(GetIID(), (LPVOID*)(&intf));
    OLECHECK_QI(hr);
  }
  return *this;
}
...
TComInterface<T, piid>::operator=(TVariant var)
{
  ...
  if (punk)
  {
    TComInterface<IUnknown, &IID_IUnknown> _cleanup(punk, false);
    HRESULT hr = punk->QueryInterface(GetIID(), (LPVOID*)(&intf));
    OLECHECK_QI(hr);
  }
  return *this;
}
...
#define _DEF_SMARTPTR(intf)           typedef TComInterface< ::intf >       intf ## Ptr
#define _DEF_SMARTPTR_IID(intf, iid)  typedef TComInterface< ::intf, &iid > intf ## Ptr
#endif

// Typedefs for StdVCL interfaces
//
namespace System { namespace Win { namespace Stdvcl
...
typedef TComInterface<System::Win::Stdvcl::IProvider>        IProviderPtr;
typedef TComInterface<System::Win::Stdvcl::IProviderDisp>    IProviderDispPtr;
typedef TComInterface<System::Win::Stdvcl::IDataBroker>      IDataBrokerPtr;
typedef TComInterface<System::Win::Stdvcl::IDataBrokerDisp>  IDataBrokerDispPtr;
...
{
  TOBJ intfObj;
  HRESULT hr = CreateRemote(machineName, intfObj);
  OLECHECK(hr);
  return intfObj;
}
...
public:
  ...
  operator TVariant& ()         { return m_Variant; }
  operator TVariantNoClear* ()  { return reinterpret_cast<TVariantNoClear*>(&m_Variant);}
  operator TVariantNoClear& ()  { return *(reinterpret_cast<TVariantNoClear*>(&m_Variant));}
...
public:
  ...
  // [lcid] param must be 'long'
  // https://msdn.microsoft.com/en-us/library/windows/desktop/aa367067(v=vs.85).aspx
  // But 'LCID' is a DWORD (https://msdn.microsoft.com/en-us/library/windows/desktop/aa383751(v=vs.85).aspx)
  operator long() const { return m_Lcid; }
...
// ==================
//
// TEventDispatcher<> offer a simple sink implementation.
//
...
public:
 ...
 ~TAutoDriver();
...
#if defined(SYSTOBJ_H)
  template <class I>
  TAutoDriver<DISPINTF>& operator=(const System::DelphiInterface<I>& src)
  {
    DISPINTF *intf = 0;
    if (src) {
  ...
  //
  void    Bind(DISPINTF *pintf, bool addRef);
...
public:
  TDispId(TAutoDriver<DISPINTF>& dispatch, LPCWSTR pName, DISPID id= DISPID_UNKNOWN) : m_DispId(id),
                                                                                       m_HResult(S_OK)
  {
    if (id == DISPID_UNKNOWN || this->m_AlwaysGetDispid)
...
TAutoDriver<DISPINTF>::TAutoDriver(const TAutoDriver<DISPINTF>  &src)
{
  ...
  // Copy Interface and Add RefCount
...
TAutoDriver<DISPINTF>::Invoke(DispatchFlag dFlags, DISPID dispid, TAutoArgsBase* args, bool hasRetVal)
{
  ...
  // Define __TRACE_ON_INVOKE to display output message after
  // a Dispatch Invoke
#if defined(__TRACE_ON_INVOKE)
...
typedef TVariantNoClear TVariantInParam;
