...
#ifdef DELPHI_AUTOREFCOUNT
	virtual int __fastcall __ObjAddRef(void);
	virtual int __fastcall __ObjRelease(void);
#endif // DELPHI_AUTOREFCOUNT
...
#ifdef DELPHI_AUTOREFCOUNT
	bool __fastcall GetDisposed(void);
	void __fastcall CheckDisposed(void);

private:
	static const int objDestroyingFlag = int(0x80000000);
	static const int objDisposedFlag = int(0x40000000);
protected:
	int FRefCount;
	static void __fastcall __MarkDestroying(const void *Obj);
	static bool __fastcall __SetDisposed(const void *Obj);

public:
	__property int RefCount = {read=FRefCount, nodefault};
	__property bool Disposed = {read=GetDisposed, nodefault};
#endif // DELPHI_AUTOREFCOUNT
};
...
#endif // !DELPHI_AUTOREFCOUNT
public:
	/* TObject.Create */ inline __fastcall TInterfacedObject(void) : TObject() { }
...
private:
	void *FController;
	_di_IInterface __fastcall GetController(void);
...
// #pragma pack(pop)
  ...
  template <class T>
  template <typename TArg>
  inline __fastcall DelphiInterface<T>::DelphiInterface(TArg&& arg,
...
public:
	/* TObject.Create */ inline __fastcall TCustomAttribute(void) : TObject() { }
	/* TObject.Destroy */ inline __fastcall virtual ~TCustomAttribute(void) { }
