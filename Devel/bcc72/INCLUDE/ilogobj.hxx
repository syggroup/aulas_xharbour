...
class IInetLogInformation : public IUnknown {
    ...
    public:
        ...
        GetSiteName(
            __inout_bcount_opt(*pcbSize) PCHAR    pszSiteName,
            IN PDWORD   pcbSize
            ) = 0;

        virtual LPSTR STDMETHODCALLTYPE
        GetComputerName(
            __inout_bcount_opt(*pcbSize) PCHAR    pszComputerName,
            IN PDWORD   pcbSize
            ) = 0;

        virtual LPSTR STDMETHODCALLTYPE
        GetClientHostName(
            __inout_bcount_opt(*pcbSize) PCHAR    pszClientHostName,
            IN PDWORD   pcbSize
            ) = 0;

        virtual LPSTR STDMETHODCALLTYPE
        GetClientUserName(
            __inout_bcount_opt(*pcbSize) PCHAR    pszClientUserName,
            IN PDWORD   pcbSize
            ) = 0;

        virtual LPSTR STDMETHODCALLTYPE
        GetServerAddress(
            __inout_bcount_opt(*pcbSize) PCHAR    pszServerIPAddress,
            IN PDWORD   pcbSize
            ) = 0;

        virtual LPSTR STDMETHODCALLTYPE
        GetOperation(
            __inout_bcount_opt(*pcbSize) PCHAR    pszOperation,
            IN PDWORD   pcbSize
            ) = 0;

        virtual LPSTR STDMETHODCALLTYPE
        GetTarget(
            __inout_bcount_opt(*pcbSize) PCHAR    pszTarget,
            IN PDWORD   pcbSize
            ) = 0;

        virtual LPSTR STDMETHODCALLTYPE
        GetParameters(
            __inout_bcount_opt(*pcbSize) PCHAR    pszParameters,
            IN PDWORD   pcbSize
            ) = 0;

        virtual LPSTR STDMETHODCALLTYPE
        GetExtraHTTPHeaders(
            __inout_bcount_opt(*pcbSize) PCHAR    pszHTTPHeaders,
            IN PDWORD   pcbSize
            ) = 0;
        ...
        virtual LPSTR STDMETHODCALLTYPE
        GetVersionString(
            __inout_bcount_opt(*pcbSize) PCHAR    pszVersionString,
            IN PDWORD   pcbSize
            ) = 0;
...
class ILogPlugin  : public IUnknown
{
    ...
    InitializeLog(
        ...
        __in_bcount(sizeof(IMDCOM)) PCHAR pvIMDCOM ) = 0;
    ...
    virtual HRESULT STDMETHODCALLTYPE
    QueryExtraLoggingFields(PDWORD cbSize, __out_ecount(*cbSize) PCHAR szParameters) = 0;
};
...
class ILogUIPlugin  : public IUnknown
{
    ...
    virtual HRESULT STDMETHODCALLTYPE
    OnProperties( __in OLECHAR* pocMachineName, __in OLECHAR* pocMetabasePath ) = 0;
};
...
public:
    virtual HRESULT STDMETHODCALLTYPE
    OnPropertiesEx(
		__in OLECHAR * pocMachineName, 
		__in OLECHAR * pocMetabasePath,
		__in OLECHAR * pocUserName,
		__in OLECHAR * pocUserPassword) = 0;
};
