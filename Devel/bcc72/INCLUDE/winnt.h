...
#ifndef _WINNT_
#pragma option push -b -a8 -pc -A- -w-pun -w-inl /*P_O_Push*/
#define _WINNT_
...
#if (defined(NONAMELESSUNION) || !defined(_MSC_EXTENSIONS)) && !defined(__CODEGEARC__)
...
#if defined(_M_MRX000) || defined(_M_ALPHA) || defined(_M_PPC) || defined(_M_IA64) || defined(_M_AMD64) || defined(_M_ARM) || defined(_M_ARM64) && !defined(__clang__) /* __CODEGEARC__ */
...
#if (defined(_M_IX86) || defined(_M_IA64) || defined(_M_AMD64) || defined(_M_ARM) || defined(_M_ARM64)) && !defined(MIDL_PASS) && !defined(__clang__) // FIXME: __CODEGEARC__
...
#if (_MSC_VER >= 1200) && !defined(MIDL_PASS) && !defined(__CODEGEARC__)
...
#if ((_MSC_VER >= 1100) || defined(__CODEGEARC__)) && defined (__cplusplus)
...
// Why isn't FORCEINLINE used everywhere?
...
/* For Embarcadero/CodeGear, let the compiler generate calls to __int64 helper functions */
    ...
    )
{
    ...
    _EDX = Bit;
    _ECX = (unsigned)Base;
    __emit__(0xF0,0x0F,0xAB,0x11);  // lock bts [ecx], edx
    __emit__(0x0F,0x92,0xC0);       // setc al
    return (BOOLEAN)_AL;
#else
    __asm {
           mov eax, Bit
...
#define POLICY_AUDIT_SUBCATEGORY_COUNT (58)
...
#define IMAGE_GUARD_FLAG_FID_SUPPRESSED               0x01       // The containing GFIDS entry is suppressed
...
#if defined(_M_AMD64) && !defined(__CODEGEARC__)
...
#define REG_APP_HIVE_OPEN_READ_ONLY     (0x00002000L)   // Open an app hive's files in read-only mode (if the hive was not previously loaded)
