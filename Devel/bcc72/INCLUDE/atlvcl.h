...
public:
  ...
  // ICPPAppServer
  HRESULT __stdcall AS_ApplyUpdates(BSTR ProviderName, VARIANT Delta, long MaxErrors, int* ErrorCount, VARIANT* OwnerData, VARIANT* AS_ApplyUpdates_result)
  {
    return AS_ApplyUpdates(WideString(ProviderName),
                           OleVariant(Delta),
                           (int)MaxErrors, *ErrorCount,
                           *(reinterpret_cast<OleVariant*>(OwnerData)),
                           *(reinterpret_cast<OleVariant*>(AS_ApplyUpdates_result)));
  }

  HRESULT __stdcall AS_GetRecords(BSTR ProviderName, long Count, int* RecsOut, long Options, BSTR CommandText, VARIANT* Params, VARIANT* OwnerData, VARIANT* AS_GetRecords_result)
  {
    return AS_GetRecords(ProviderName, (int)Count, *RecsOut, (int)Options, CommandText,
                         *(reinterpret_cast<OleVariant*>(Params)),
                         *(reinterpret_cast<OleVariant*>(OwnerData)),
                         *(reinterpret_cast<OleVariant*>(AS_GetRecords_result)));
  }

  HRESULT __stdcall AS_DataRequest(BSTR ProviderName, VARIANT Data, VARIANT* AS_DataRequest_result)
  {
    return AS_DataRequest((WideString)ProviderName,
                           OleVariant(Data),
                          *(reinterpret_cast<OleVariant*>(AS_DataRequest_result)));
  }

  HRESULT __stdcall AS_GetProviderNames(VARIANT* AS_GetProviderNames_result)
  {
    return AS_GetProviderNames(*(reinterpret_cast<OleVariant*>(AS_GetProviderNames_result)));
  }

  HRESULT __stdcall AS_GetParams(BSTR ProviderName, VARIANT* OwnerData, VARIANT* AS_GetParams_result)
  {
    return AS_GetParams(WideString(ProviderName), *(reinterpret_cast<OleVariant*>(OwnerData)),
                        *(reinterpret_cast<OleVariant*>(AS_GetParams_result)));
  }

  HRESULT __stdcall AS_RowRequest(BSTR ProviderName, VARIANT Row, long RequestType, VARIANT* OwnerData, VARIANT* AS_RowRequest_result)
  {
    return AS_RowRequest(WideString(ProviderName),
                         OleVariant(Row),
                         (int)RequestType,
                         *(reinterpret_cast<OleVariant*>(OwnerData)),
                         *(reinterpret_cast<OleVariant*>(AS_RowRequest_result)));
  }

  HRESULT __stdcall AS_Execute(BSTR ProviderName, BSTR CommandText, VARIANT* Params, VARIANT* OwnerData)
  {
    return AS_Execute(WideString(ProviderName),
...
TVclComControl<T, TVCL>::Initialize()
{
...
#else
  m_VclCtl = (TWinControlAccess<TVCL>*)(TVCL::CreateParentedControl(hwndParkingWindow));
#endif
  ...
  if (m_VclCtl->ControlStyle.Contains(csReflector))
    reinterpret_cast<TWinControl*>(m_VclWinCtl) = ::CreateReflectorWindow(hwndParkingWindow, m_VclCtl);
  else
    reinterpret_cast<TWinControl*>(m_VclWinCtl) = (TWinControl*)m_VclCtl;
 ...
 ~TVclControlImpl()
  ...
  STDMETHOD(OnAmbientPropertyChange)(DISPID dispid)
  {
    ...
    if (dispid == DISPID_AMBIENT_BACKCOLOR || dispid == 0)
    {
      ...
      if (hres != S_OK)
        return hres;
      m_VclWinCtl->Perform(CM_PARENTCOLORCHANGED, 1,
                           LPARAM(arg.GetRetVariant()));
    }
...
class TPropertyPageImplHack : public Axctrls::TPropertyPageImpl, public IUnknown
{
...
public:
  HRESULT __stdcall QueryInterface(const GUID &IID, void **Obj)
  {
    if (GetInterface(IID, Obj))
      ...
      return Axctrls::TPropertyPageImpl::QueryInterface(IID, Obj);
  }
  int __stdcall _AddRef(void)
  {
    return TPropertyPageImpl::_AddRef();
  }
  int __stdcall _Release(void)
  {
    return TPropertyPageImpl::_Release();
  }
public:
  __fastcall TPropertyPageImplHack(const System::_di_IInterface Controller) : Axctrls::TPropertyPageImpl(Controller)
  { }
  __fastcall virtual ~TPropertyPageImplHack(void)
  { }
};
