...
#include <System.Win.ComConst.hpp>
...
#define DECLARE_PROGID(progid)        static LPCTSTR GetProgID()           { return _T(progid);}
#define DECLARE_DESCRIPTION(desc)     static LPCTSTR GetDescription()      { return _T(desc);  }
...
public:
  ...
  //
  TATLModule(System::Sysutils::TProcedure InitProcedure);
#endif
 ...
 ~TATLModule();
  ...
  static  System::Sysutils::TProcedure SaveInitProc;   // Holds system's old InitProcedure
...
template <class T>
TATLModule<T>::TATLModule(System::Sysutils::TProcedure initProcedure): m_ThreadID(0), m_bRun(true),
                                                                       m_bExe(true), m_bAutomationServer(false)
{
  ...
  {
    SaveInitProc = System::Sysutils::TProcedure(System::InitProc);
...
{
  System::Sysutils::TSysCharSet DelimSet;
  DelimSet << '/' << '-';

  if (System::Sysutils::FindCmdLineSwitch("REGSERVER", DelimSet, true))
  {
    RegisterServer(TRUE);
    m_bRun = false;
  }
  else if (System::Sysutils::FindCmdLineSwitch("UNREGSERVER", DelimSet, true))
  {
    UnregisterServer();
    m_bRun = false;
  }
  ...
  else if (!(System::Sysutils::FindCmdLineSwitch("AUTOMATION", DelimSet, true) ||
             System::Sysutils::FindCmdLineSwitch("EMBEDDING", DelimSet, true)))
  {
    RegisterServer(TRUE);
  }
#endif
...
LONG TATLModule<T>::Unlock()
{
  ...
  if ((result == 0) && m_bExe)
  {
    ...
    System::Sysutils::TSysCharSet DelimSet;
    DelimSet << '/' << '-';
    if (System::Sysutils::FindCmdLineSwitch("AUTOMATION", DelimSet, true))
...
//
template <class T>
System::Sysutils::TProcedure TATLModule<T>::SaveInitProc = 0;
...
bool __fastcall TATLModule<T>::AutomationTerminateProc()
{
  ...
  if (_Module.GetLockCount() > 0)
  {
    ...
    return ::MessageBoxA(0, (System_Win_Comconst_SNoCloseActiveServer1 + System_Win_Comconst_SNoCloseActiveServer2).c_str(),
