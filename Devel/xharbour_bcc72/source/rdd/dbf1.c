/*
 * $Id: dbf1.c 10164 2016-10-11 00:39:53Z lculik $
 */
...
static void hb_dbfUpdatedbaselockValue( DBFAREAP pArea, ULONG ulRecNo )
{
   if( pArea->uidbaselock && ulRecNo )
   {
                         ...
                         ( HB_FOFFSET ) pArea->uiRecordLen ) == ( ULONG ) pArea->uiRecordLen )
      {
         ...
         hb_fileWriteAt( pArea->pDataFile, pPtr, pArea->area.lpFields[ pArea->uidbaselock ].uiLen - 3,
                         ( HB_FOFFSET ) pArea->uiHeaderLen +
   ...
   *pulBlock = *pulSize = *pulType = 0;
   ...
   if( uiIndex >= pArea->area.uiFieldCount ||
       ( pArea->area.lpFields[ uiIndex ].uiType != HB_FT_MEMO &&
         pArea->area.lpFields[ uiIndex ].uiType != HB_FT_BINARY &&
         pArea->area.lpFields[ uiIndex ].uiType != HB_FT_PICTURE &&
   ...
   else
   {
      ...
      else if( HB_IS_DATETIME( pItem ) )
      {
                  ...
                  pField->uiType == HB_FT_TIME )
         {
            ...
            if( pField->uiType == HB_FT_DATETIME || pField->uiType == HB_FT_TIMESTAMP )
            {
