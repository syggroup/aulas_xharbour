/*
 * $Id: expropt2.c 10074 2014-12-15 12:35:31Z zsaulius $
 */
...
BOOL hb_compExprReduceUPPER( PHB_EXPR pSelf, HB_MACRO_DECL )
{
   ...
   if( pArg->ExprType == HB_ET_STRING )
   {
      ...
      if( ulLen == 0 )
      {
         ...
         if( fLower )
         {
            ...
            else
            {
               ...
               else
               {
                  ...
                  HB_MEMCPY( szValue, pArg->value.asString.string, ( size_t ) ( pArg->ulLength + 1 ) );
                  fDealloc = TRUE;
               }
               do
               {
         ...
         pExpr = hb_compExprNewString( szValue, pArg->ulLength, fDealloc );
