/*
 * $Id: gtwvt.c 10164 2016-10-11 00:39:53Z lculik $
 */
...
#define HB_WVT_LOCK    
#define HB_WVT_UNLOCK  
#define HB_ITEMGETSTR( itm, phstr, plen )     hb_strget( itm, phstr, plen )
...
static BOOL hb_iswinvista( void )
{
   OSVERSIONINFO osvi;
   BOOL bRet = FALSE;
...
static BOOL hb_iswin9x( void )
{
   ...
   if( GetVersionEx( &osvi ) )
   {
      bRet = ( osvi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS );
   }
...
//                      break.
               ...
               if( j > 0 )
               {
                  ...
                  hb_gt_w32_setClipboard( CF_UNICODETEXT, hb_itemGetCPtr(pItem ),hb_itemGetCLen(pItem));
                  hb_itemRelease( pItem );
               }
               hb_xfree( sBuffer );
#else
               if( j > 0 )
               {
                  ...
                  hb_gt_w32_setClipboard( pWVT->CodePage == OEM_CHARSET ?
                                             CF_OEMTEXT : CF_TEXT, hb_itemGetCPtr(pItem),hb_itemGetCLen(pItem) );
                  hb_itemRelease( pItem );
               }
               else
                  hb_xfree( sBuffer );
      ...
      case WM_SYSKEYDOWN:
      {
         ...
         switch( wParam )
         {
            case VK_RETURN:
               ...
               {
                  pWVT->IgnoreWM_SYSCHAR = TRUE;   /* this must be FIRST, otherwise some process in hb_gt_wvt_FullScreen posts the ENTER key to the InputQueue */
...
static BOOL hb_gt_wvt_Info( PHB_GT pGT, int iType, PHB_GT_INFO pInfo )
{
   ...
   switch( iType )
   {
      ...
      case HB_GTI_CLIPBOARDDATA:
         if( hb_itemType( pInfo->pNewVal ) & HB_IT_STRING )
#if defined( UNICODE )
            hb_gt_w32_setClipboard( CF_UNICODETEXT, hb_itemGetCPtr(pInfo->pNewVal) ,hb_itemGetCLen(pInfo->pNewVal));
#else
            hb_gt_w32_setClipboard( pWVT->CodePage == OEM_CHARSET ?
                                       CF_OEMTEXT : CF_TEXT, hb_itemGetCPtr(pInfo->pNewVal ),hb_itemGetCLen(pInfo->pNewVal));
#endif
         else
         {
...
#if defined( UNICODE )
//             hb_gt_w32_getClipboard( CF_UNICODETEXT, hb_itemGetCPtrpInfo->pResult );
#else
