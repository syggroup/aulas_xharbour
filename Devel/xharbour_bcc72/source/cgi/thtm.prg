/*
 * $Id: thtm.prg 10058 2014-12-02 12:13:20Z zsaulius $
 */
...
METHOD PutImageUrl( cImage, nBorder, nHeight, cUrl, ;
   ...
   IF nBorder != NIL
      cStr += " border = " + iif( HB_ISNUMERIC( nBorder ), NTRIM( nBorder ), nBorder ) + CRLF()
   ENDIF
...
METHOD Marquee( cText, cFont, cFntColor, nFntSize, ;
   ...
   ::cStr +=  'loop=' + iif( HB_ISNUMERIC( loop ), NTRIM( loop ), loop ) + ' '
...
METHOD StartMarquee( cFont, cFntColor, nFntSize, ;
   ...
   cStr += '<MARQUEE align="' + cAlign + '" ' + ;
      ...
      'loop=' + iif( HB_ISNUMERIC( loop ), NTRIM( loop ), loop ) + ' ' + ;
...
FUNCTION HTMLANY2STR( xVal )
   ...
   SWITCH ValType( xVal )
   CASE "C"
      RETURN iif( Empty( xVal ), ".", xVal )
   CASE "N"
      RETURN AllTrim( Str( xVal ) )
   CASE "O"
      RETURN "<" + xVal:CLASSNAME() + ">"
   CASE "D"
      RETURN DToC( xVal )
   CASE "L"
      RETURN LTOC( xVal )
