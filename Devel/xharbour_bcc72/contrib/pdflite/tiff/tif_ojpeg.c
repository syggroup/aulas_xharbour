...
+ * encoder's/decoder's DCcoefficients for each image component are reset before
...
+ *
+ * This kludge isnot sanctioned or supported by the Independent JPEG Group, and
 ...
 */
    ...
    {
      TIFFTAG_JPEGIFOFFSET          ,1            ,1            , TIFF_LONG,
      FIELD_JPEGIFOFFSET          ,FALSE,FALSE,"JPEGInterchangeFormat"
    },
    {
      TIFFTAG_JPEGIFBYTECOUNT       ,1            ,1            , TIFF_LONG,
      FIELD_JPEGIFBYTECOUNT       ,FALSE,FALSE,"JPEGInterchangeFormatLength"
    },
    {
      TIFFTAG_JPEGRESTARTINTERVAL   ,1            ,1            , TIFF_SHORT,
      FIELD_JPEGRESTARTINTERVAL   ,FALSE,FALSE,"JPEGRestartInterval"
    },
    {
      TIFFTAG_JPEGLOSSLESSPREDICTORS,TIFF_VARIABLE,TIFF_VARIABLE, TIFF_SHORT,
      FIELD_JPEGLOSSLESSPREDICTORS,FALSE,TRUE ,"JPEGLosslessPredictors"
    },
    {
      TIFFTAG_JPEGPOINTTRANSFORM    ,TIFF_VARIABLE,TIFF_VARIABLE, TIFF_SHORT,
      FIELD_JPEGPOINTTRANSFORM    ,FALSE,TRUE ,"JPEGPointTransforms"
    },
    {
...
static const char JPEGLib_name[]={"OJPEG"},
...
{
    OJPEGState *sp = (OJPEGState *) cinfo;	/* NB: cinfo assumed first */
    char buffer[JMSG_LENGTH_MAX];
    TIFF* tif = sp->tif;
#ifdef PDFLIB_ALIGN16
    jmp_buf jbuf;
#endif
    ...
    _TIFFError(tif, JPEGLib_name,buffer); /* Display error message */
    jpeg_abort(cinfo); /* Clean up JPEG Library state */
#ifdef PDFLIB_ALIGN16
    memcpy(jbuf, ((OJPEGState *) cinfo)->exit_jmpbuf, sizeof (jmp_buf));
    LONGJMP(jbuf, 1); 				/* Return to TIFF client */
#else
    LONGJMP(((OJPEGState *)cinfo)->exit_jmpbuf,1); /* Return to TIFF client */
#endif
}

/* TODO: add custom error handler with logging */
static void
TIFFojpeg_output_message(register j_common_ptr cinfo)
  { char buffer[JMSG_LENGTH_MAX];
    OJPEGState* sp = (OJPEGState*) cinfo;
    TIFF* tif = sp->tif;
 ...
 */
    (*cinfo->err->format_message)(cinfo,buffer);
    _TIFFWarning(tif, JPEGLib_name,buffer);
  }
...
#define	CALLJPEG(sp, fail, op)					\
	(SETJMP(pdflib_jbuf) ? (fail) :				\
	    (memcpy((sp)->exit_jmpbuf, pdflib_jbuf, sizeof (jmp_buf)), (op)))
 ...
 */
    ...
    if (!(sp->jpegtables = (void*)
       ...
       )
      {
        ...
        _TIFFError(tif, "TIFFojpeg_tables_dest",no_jtable_space);
        return 0;
      };
...
_notSupported(register TIFF *tif)
  { const TIFFCodec *c = TIFFFindCODEC(tif->tif_dir.td_compression);
    ...
    _TIFFError(tif, tif->tif_name,"%s compression not supported",c->name);
    return 0;
  }
#endif /* never */
...
/* "Dummy" stub */
std_term_source(register j_decompress_ptr cinfo){(void) cinfo;}
 ...
 */
    if (cc % sp->bytesperline) _TIFFWarning(tif, tif->tif_name,no_write_frac);
...
#                         endif
        ...
        if (++sp->scancount >= size)
          {
            if (CALLJPEG(sp,-1,jpeg_write_raw_data(&sp->cinfo.c,
		    sp->ds_buffer,lines_per_MCU)) != lines_per_MCU
               ) return 0;
            sp->scancount = 0;
          };
        ++tif->tif_row++
 ...
 */
    if (td->td_bitspersample != 8)
      {
        _TIFFError(tif, module,bad_bps,td->td_bitspersample);
        status = 0;
      };
 ...
 */
    switch (td->td_photometric)
       ...
       */
          if (   (   td->td_ycbcrsubsampling[0] == 1
            ...
            sp->cinfo.c.raw_data_in =
              ((sp->h_sampling = (unsigned char)(td->td_ycbcrsubsampling[0]))<<3
              | (sp->v_sampling = (unsigned char)(td->td_ycbcrsubsampling[1]))
              ) != 011;
          else
            {
              _TIFFError(tif, module,bad_subsampling);
              status = 0;
            };
      ...
      L2: sp->cinfo.d.in_color_space = sp->cinfo.d.jpeg_color-space;
          break;
        default                    :
          _TIFFError(tif, module,bad_photometry,td->td_photometric);
          status = 0;
      };
...
#       endif
		...
		"JPEG tile height must be multiple of %d",size);
            status = 0;
          };
#       ifdef C_LOSSLESS_SUPPORTED
...
#       endif
        if ((segment_width = td->td_tilewidth) % size)
          {
            _TIFFError(tif, module,
		"JPEG tile width must be multiple of %d",size);
            status = 0;
          };
...
#       endif
        if (td->td_rowsperstrip < (segment_height = td->td_imagelength))
          {
            if (td->td_rowsperstrip % size)
              {
		_TIFFError(tif, module,
			"JPEG RowsPerStrip must be multiple of %d",size);
                status = 0;
              };
    ...
    if (segment_width > 65535 || segment_height > 65535)
      {
        _TIFFError(tif, module,"Strip/tile too large for JPEG");
        status = 0;
      };
    ...
    switch (sp->jpegtablesmode & (JPEGTABLESMODE_HUFF|JPEGTABLESMODE_QUANT))
      { register JHUFF_TBL *htbl;
        ...
        case JPEGTABLESMODE_HUFF                     :
          if (!CALLVJPEG(sp,jpeg_set_quality(&sp->cinfo.c,
		sp->jpegquality,FALSE)))
            return 0;
...
/*ARGSUSED*/ static int
OJPEGDecode(register TIFF *tif,tidata_t buf,tsize_t lcc,tsample_t s)
  { tsize_t bytesperline = isTiled(tif)
    ...
    /*
     * PDFlib GmbH KLUDGE alert: this sick stuff is an attempt to
     * work around a compiler bug in gcc 3.2.3 for Linux on Itanium
     */
    volatile tsize_t cc = lcc;
...
#   endif
    while (--cc >= 0)
      { int clumpoffset, ci;
        ...
        if (sp->scancount >= (int)size) /* reload downsampled-data buffers */
          {
            if (   CALLJPEG(sp,-1,jpeg_read_raw_data(&sp->cinfo.d,
		    sp->ds_buffer,lines_per_MCU)) != lines_per_MCU
               ) return 0;
            sp->scancount = 0;
          };
...
#   else
                  * (size = DCTSIZE);
#   endif
 L: if (sp->scancount >= (int)size) /* reload downsampled-data buffers */
      {
        if (CALLJPEG(sp,-1,jpeg_read_raw_data(&sp->cinfo.d,
		    sp->ds_buffer,lines_per_MCU)) != lines_per_MCU
           ) return 0;
...
fake_SOS_marker(j_decompress_ptr cinfo){(void) cinfo; return JPEG_REACHED_SOS;}

/*ARGSUSED*/ METHODDEF(int)
suspend(j_decompress_ptr cinfo){(void) cinfo; return JPEG_SUSPENDED;}
...
OJPEGSetupDecode(register TIFF *tif)
  { static char module[]={"OJPEGSetupDecode"};
    J_COLOR_SPACE jpeg_color_space = JCS_YCbCr, /* shut up compiler warning */
    		/* Color space of JPEG-compressed image */
                  out_color_space = JCS_RGB; /* shut up compiler warning */
    ...
    if (isTiled(tif))
      {
        if (sp->is_WANG) /* we don't know how to handle it */
          {
            _TIFFError(tif, module,"Tiled Wang image not supported");
            return 0;
          };
...
#       endif /* someday */
     ...
     */
     ...
     L: if (p - (unsigned char *)sp->jpegtables > 2) /* fake "JPEGTables" */
        ...
        if (   sp->cinfo.d.image_width  != segment_width
          ...
          {
            _TIFFError(tif, module,"Improper JPEG strip/tile size");
            return 0;
          };
        if (sp->cinfo.d.num_components != td->td_samplesperpixel)
          {
            _TIFFError(tif, module,"Improper JPEG component count");
            return 0;
          };
        if (sp->cinfo.d.data_precision != td->td_bitspersample)
          {
            _TIFFError(tif, module,"Improper JPEG data precision");
            return 0;
          };
        ...
        do
          {
            if (   ( sp->cinfo.d.comp_info[i].h_samp_factor << 3
              ...
              {
                _TIFFError(tif, module,"Improper JPEG subsampling factors");
                return 0;
              };
     ...
     */
        ...
        switch (jpeg_color_space)
	      ...
	      break;
	    case JCS_YCCK    :
	    /* shut up compiler warning "enumeration JCS_YCCK not handled" */
	      break;
          };
          ...
          if (v32 && td->td_photometric == PHOTOMETRIC_YCBCR )
	  {
            ...
            if (td->td_refblackwhite)
              { register long top = ( 1 << td->td_bitspersample );
                ...
                td->td_refblackwhite[2] = td->td_refblackwhite[4]
		    = (float)(top >> 1);
              }
            else
              {
...
#define PDF_Wang_Labs "\x57\x61\x6E\x67\x20\x4C\x61\x62\x73" /* "Wang Labs" */
#define PDF_xVCS_V3_5 "\x78\x56\x43\x53\x20\x56\x33\x2E\x35" /* "xVCS V3.5" */

            v32 = (*sp->vsetparent)(tif,tag,ap);
            if( TIFFGetField( tif, TIFFTAG_SOFTWARE, &software )
                && (strstr( software, PDF_Wang_Labs) ||
/* PDFlib GmbH: these files also require WANG-specific treatment */
		    strstr( software, PDF_xVCS_V3_5) ))
                sp->is_WANG = 1;
            return v32;
        }
    ...
    if(v32 !=0){
		...
		break;
            case TIFFTAG_JPEGLOSSLESSPREDICTORS:
		sp->jpeglosslesspredictors_length=(uint16)v32;
		break;
            case TIFFTAG_JPEGPOINTTRANSFORM:
    ...
    switch (tag)
     ...
     /* Validate the JPEG-process code. */
        ...
        case TIFFTAG_JPEGPROC              :
          switch (v32)
            {
              default               : _TIFFError(tif, tif->tif_name,
...
#             else /* not C_LOSSLESS_SUPPORTED */
              case JPEGPROC_LOSSLESS: _TIFFError(tif, JPEGLib_name,
     ...
     */
     ...
     /* We have a vector of offsets to quantization tables, so load 'em! */
        ...
        case TIFFTAG_JPEGQTABLES           :
          if (v32)
            { uint32 *v;
              int i;
	      if (v32 > td->td_samplesperpixel)
	      {
		  _TIFFError(tif, tif->tif_name,
		    "Too many quantization tables, ignoring excess tables");
		    /* Read only required number of tables */
		    v32 = td->td_samplesperpixel;
	      }

              if (v32 > NUM_QUANT_TBLS)
                {
                  _TIFFError(tif, tif->tif_name,"Too many quantization tables");
                  return 0;
                };
              i = 0;
              v = va_arg(ap,uint32 *);
                ...
                bufoff=0;
                for(i2=0;(uint16)i2<sp->jpegqtables_length;i2++){
                    TIFFSeekFile(tif, v[i2], SEEK_SET);
                    TIFFReadFile(tif, &(((unsigned char*)
			(sp->jpegqtables))[bufoff]),
				 64);
                    bufoff+=64;
                }
     ...
     /* We have a vector of offsets to AC Huffman tables, so load 'em! */
       ...
       L: if (v32)
		  ...
		  {
		      _TIFFError(tif, tif->tif_name,
			"Too many DC Huffman tables, ignoring excess tables");
			/* Read only required number of tables */
			v32 = sp->jpegdctables_length = td->td_samplesperpixel;
		  } else
		  {
		      _TIFFError(tif, tif->tif_name,
			"Too many AC Huffman tables, ignoring excess tables");
			/* Read only required number of tables */
			v32 = sp->jpegactables_length = td->td_samplesperpixel;
		  }
	      }
              if (v32 > NUM_HUFF_TBLS)
                {
                  _TIFFError(tif, tif->tif_name,"Too many Huffman tables");
                  return 0;
                };
              v = va_arg(ap,uint32 *);
                if(tag == TIFFTAG_JPEGDCTABLES) {
		sp->jpegdctables=_TIFFmalloc(272*sp->jpegdctables_length);
                    ...
                    bufoff=0;
                    code_count=0;
                    for(i2=0;(uint16)i2<sp->jpegdctables_length;i2++){
                        TIFFSeekFile(tif, v[i2], SEEK_SET);
                        TIFFReadFile(tif,
			     &(((unsigned char*)(sp->jpegdctables))[bufoff]),
				     16);
                        code_count=0;
                        for(k2=0;k2<16;k2++){
                            code_count+=((unsigned char*)
				(sp->jpegdctables))[k2+bufoff];
                        }

			/* PDFlib GmbH: some files have really broken
			 * Huffman tables; avoid a crash.
			 */
			if (code_count > 256)
			{
                            _TIFFError(tif, JPEGLib_name,
                                       "DC Huffman table corrupt");
			    return 0;
			}
                        TIFFReadFile(tif,
		...
		sp->jpegactables=_TIFFmalloc(272*sp->jpegactables_length);
                    ...
                    bufoff=0;
                    code_count=0;
                    for(i2=0;(uint16)i2<sp->jpegactables_length;i2++){
                        TIFFSeekFile(tif, v[i2], SEEK_SET);
                        TIFFReadFile(tif,
			  &(((unsigned char*)(sp->jpegactables))[bufoff]), 16);
                        code_count=0;
                        for(k2=0;k2<16;k2++){
                            code_count+=
			       ((unsigned char*)(sp->jpegactables))[k2+bufoff];
                        }
			...
			{
                            _TIFFError(tif, JPEGLib_name,
                                       "AC Huffman table corrupt");
			    return 0;
			}

                        TIFFReadFile(tif, &(((unsigned char*)
			        (sp->jpegactables))[bufoff+16]), code_count);
                        bufoff+=16;
                        bufoff+=code_count;
                    }
...
OJPEGPrintDir(register TIFF *tif,FILE *fd,long flags)
  { register OJPEGState *sp = OJState(tif);
    ...
    if (   ( flags & (TIFFPRINT_JPEGQTABLES|
		TIFFPRINT_JPEGDCTABLES|TIFFPRINT_JPEGACTABLES)
           )
        && sp->jpegtables_length
       )
      fprintf(fd,"  JPEG Table Data: <present>, %lu bytes\n",
        (long unsigned int) sp->jpegtables_length);
  }
#endif /* PDFLIB_TIFFWRITE_SUPPORT */
    ...
    if ( sp )
      {
        ...
        if (sp->jpegactables) {
	    _TIFFfree(sp->jpegactables);sp->jpegactables=0;
	}
        if (sp->jpegdctables) {
	    _TIFFfree(sp->jpegdctables);sp->jpegdctables=0;
	}
...
#   endif /* never */
     ...
     */
        ...
        if (!tif->tif_base)
          ...
          {
            _TIFFError(tif, tif->tif_name,"Cannot read file from memory map");
            return 0;
          }
 ...
 */
    TIFFMergeFieldInfo(tif,ojpegFieldInfo,
