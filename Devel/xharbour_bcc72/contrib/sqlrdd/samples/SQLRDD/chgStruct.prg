...
Function Main( cRDD, cDSN )
   ...
   local aStruct := {{"CODE_ID","C",8,0,.F.,,,,,1 },{"CARDID","C",1,0},{"DESCR","C",50,0},{"PERCENT","N",10,2},{"DAYS","N",8,0},{"DATE_LIM","D",8,0},{"OBS","M",10,0}}
   local nCnn, i, s, r, c
   ...
   r = row()
   c = col()
   browse()  // ATÉ MOSTRA TUDO CERTINHO POREM SEM O CAMPO LOGICO AINDA
   setpos( r,c)
   ...
   r = row()
   c = col()
   ALERT('agora vai aplicar o filtro e deveria mostrar todos os registros, por que todos ainda sao falso')

   SR_SETFILTER('enable=FALSE')
//   SR_SETFILTER('enable='+ sr_cdbvalue(.F.)) // TENTEI ASSIM TMB. E NÃO DEU
   dbgotop()
   browse()
   setpos( r,c)
