/*
 * $Id: ssl.c 10175 2016-12-26 00:40:29Z lculik $
 */
...
#if defined( HB_OS_WIN ) && \
    defined( HB_CPU_X86 ) && \
    OPENSSL_VERSION_NUMBER >= 0x00908000L &&  OPENSSL_VERSION_NUMBER <= 0x1000206fL
...
HB_FUNC( SSL_NEW )
{
   if( hb_SSL_CTX_is( 1 ) )
   {
      ...
      if( ctx )
      {
         PHB_SSL * ph = ( PHB_SSL * ) hb_gcAlloc( sizeof( PHB_SSL * ), SSL_release );
...
HB_FUNC( SSL_DUP )
{
   if( hb_SSL_is( 1 ) )
   {
      ...
      if( ssl_par )
      {
         PHB_SSL * ph = ( PHB_SSL * ) hb_gcAlloc( sizeof( SSL * ), SSL_release );
