/*
 * $Id: postgres.c 9752 2012-10-14 03:33:16Z andijahja $
 */
...
void hb_PGresult_ret( PGresult * p )
{
   if( p )
   {
      void ** ph = ( void ** ) hb_gcAlloc( sizeof( PGresult * ), PGresult_release );
...
{
   void ** ph = ( void ** ) hb_parptrGC( PGresult_release, iParam );
...
static void hb_PGcancel_ret( PGcancel * p )
{
   if( p )
   {
      void ** ph = ( void ** ) hb_gcAlloc( sizeof( PGcancel * ), PGcancel_release );
...
{
   void ** ph = ( void ** ) hb_parptrGC( PGcancel_release, iParam );
...
static void hb_FILE_ret( FILE * p )
{
   if( p )
   {
      void ** ph = ( void ** ) hb_gcAlloc( sizeof( FILE * ), FILE_release );
...
{
   void ** ph = ( void ** ) hb_parptrGC( FILE_release, iParam );
