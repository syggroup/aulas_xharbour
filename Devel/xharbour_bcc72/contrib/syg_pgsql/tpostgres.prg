/*
 * $Id: tpostgres.prg 9749 2012-10-13 04:18:06Z andijahja $
 */
...
CREATE CLASS TPQServer
   ...
   METHOD   New( cHost, cDatabase, cUser, cPass, nPort, Schema, nTimeOut )
   METHOD   Destroy()
...
METHOD New( cHost, cDatabase, cUser, cPass, nPort, Schema, nTimeOut ) CLASS TPQserver
   LOCAL res
   ...
   ::pDB := PQconnectDB( "dbname = " + cDatabase + " host = " + cHost + " user = " + cUser + " password = " + cPass + " port = " +  hb_ntos( nPort )+' connect_timeout = ' +  hb_ntos( nTimeOut ) )
...
METHOD Struct(lfieldname) CLASS TPQquery
    Local result := {}
    Local i

    For i := 1 to Len(::aStruct)
      if lfieldname
         aadd( result, ::aStruct[i, 1] )
      else
...
Function Syg_DataToSql(xField,lDATA)
Local cType:='', result := 'NULL'

IF VALTYPE(xField)='D' .AND. DAY(xField)<=0 .AND. lDATA=NIL
   lDATA:=.T.
ENDIF
IF lDATA=NIL
   lDATA:=.F.
...
Local temp, aTemp, oQuery, __lret:=.t.

oQuery := oServer:Query( cQuery ) // exeuta uma query
IF oQuery:neterr()
   __lret:=.f.
   //ShowMsg('Erro:'+ oQuery:ErrorMsg())
   oServer:cError := 'Erro:'+ oQuery:ErrorMsg()
ELSE
   IF aColunas#nil
      aColunas:=oQuery:Struct(.t.) // somente nome das colunas
   endif

   if aResult#Nil
      __nTOTCOL:=oQuery:nFields
      __nTOTLIN:=oQuery:nLastrec

      aResult := {}
      for x := 1 to __nTOTLIN
          aTemp := {}
          for y := 1 to __nTOTCOL
             temp := oQuery:FieldGet( y , x )
             aadd( aTemp, iif( temp == NIL, "", temp ) )
          next
          aadd( aResult, aTemp)
      next
