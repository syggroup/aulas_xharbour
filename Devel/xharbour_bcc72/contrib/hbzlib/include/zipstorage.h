/*
 * $Id: zipstorage.h 9685 2012-09-16 05:42:35Z andijahja $
 */
...
//       m_uTotalSoFar = 0; // already done in CZipArchive::CZipClbckStrg::Get
   ...
   DWORD LeftToDo()
   {
      return m_uTotalToDo - m_uTotalSoFar;
   }
   ...
   int GetStep()
   {
      return m_iStep ? m_iStep : 1;
   }                                                 // do not allow 0 (crash)
...
{
   return ( DWORD ) ( m_pFile->GetPosition() ) + m_uBytesInWriteBuffer;
}
