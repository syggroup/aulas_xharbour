...
// $Revision: 9685 $ $Name:  $
// $Date: 2012-09-15 22:42:35 -0700 (Sat, 15 Sep 2012) $ $Author: andijahja $
////////////////////////////////////////////////////////////////////////////////
...
CZipString( const stdbs&stringSrc )
{
   assign( stringSrc );
}
CZipString( LPCTSTR lpsz )
{
   if( ! lpsz )
      Empty();
   else
...
{
   resize( nNewLength > -1 ? nNewLength : zslen( c_str() ) );
}
...
CZipString Left( int nCount ) const
{
...
CZipString Mid( int nFirst ) const
{
   return substr( nFirst );
}
CZipString Mid( int nFirst, int nCount ) const
{
