/*
* $Id: gtwvw.c 10164 2016-10-11 00:39:53Z lculik $
 */
/*
 * GTWVW.C
...
static void  hb_gt_wvw_vxPutch( WIN_DATA * pWindowData, USHORT iRow, USHORT iCol, BYTE bColor, BYTE bChar );
static void  hb_gt_wvw_usBox( WIN_DATA * pWindowData, int iTop, int iLeft, int iBottom, int iRight, BYTE * pbyFrame, BYTE bColor );
...
static BOOL hb_gt_wvw_KeyEvent( WIN_DATA * pWindowData, UINT message, WPARAM wParam, LPARAM lParam ,UINT  usWinNum, HWND hWnd);
...
static void hb_gt_wvw_Init( PHB_GT pGT, FHANDLE hFilenoStdin, FHANDLE hFilenoStdout, FHANDLE hFilenoStderr )
{
...
//       memset(s_pWvwData->s_sApp, 0, sizeof( APP_DATA ) ) ;
      ...
      strcpy( s_pWvwData->szAppName, "xHarbour WVW" );
      strcpy( s_pWvwData->szSubWinName , "xHarbour WVW subwindows" );
...
BOOL HB_EXPORT hb_gt_wvwDestroyPicture( IPicture * iPicture )
{
   BOOL bResult = FALSE;
    ...
    for (j= (int) (s_pWvwData->s_usNumWindows-1); j >= 0; j--)
    {
       ...
       if ( pWindowData->hWnd )
       {
         ...
         */
         ...
         {
           CONTROL_DATA *pcd     = pWindowData->pcdCtrlList->pNext;
...
static void hb_gt_wvw_Replicate( PHB_GT pGT, int iRow, int iCol, BYTE bColor, BYTE bAttr, USHORT usChar, HB_SIZE ulLen )
{
  HB_TRACE( HB_TR_DEBUG, ( "hb_gt_wvw_Replicate( %hu, %hu, %i, %i, %lu )", iRow, iCol, bColor, bAttr, usChar, ulLen ) );
...
static void hb_gt_wvw_PutText( PHB_GT pGT, int iRow, int iCol, BYTE bColor, BYTE * pText, HB_SIZE ulLen )
{
  HB_TRACE( HB_TR_DEBUG, ( "hb_gt_wvw_PutText(%hu, %hu, %p, %lu, %d)", iRow, iCol, pText, ulLen, bColor));
...
static void hb_gt_wvw_SetAttribute( PHB_GT pGT, int iTop, int iLeft, int iBottom, int iRight, BYTE bColor )
{
  HB_TRACE( HB_TR_DEBUG, ( "hb_hb_gt_wvw_SetbColoribute( %hu, %hu, %hu, %hu, %d", iTop, iLeft, iBottom, iRight, ( int ) bColor ) );
...
static void hb_gt_wvw_Scroll(PHB_GT pGT, int iTop, int iLeft, int iBottom, int iRight, BYTE bColor, BYTE bChar, int iRows, int iCols )
{
  ...
  HB_TRACE( HB_TR_DEBUG, ( "hb_gt_wvw_Scroll( %hu, %hu, %hu, %hu, %d, %hd, %hd )", iTop, iLeft, iBottom, iRight, ( int ) bColor, iRows, iCols ) );
...
/*-------------------------------------------------------------------*/
static void hb_gt_wvw_WriteAt( PHB_GT pGT, int iRow, int iCol, BYTE * pText, HB_SIZE ulLength )
{
...
static void hb_gt_wvw_Box( PHB_GT pGT, int iTop, int iLeft, int iBottom, int iRight, BYTE * pbyFrame, BYTE bColor )
{
...
static void hb_gt_wvw_HorizLine( PHB_GT pGT, int iRow, int iLeft, int iRight, BYTE bChar, BYTE bColor )
{
...
static void hb_gt_wvw_VertLine( PHB_GT pGT, int iCol, int iTop, int iBottom, BYTE bChar, BYTE bColor )
{
    int i_Width ;
    int i_Height;
    int i_Row;
...
static void hb_gt_wvw_OutStd( PHB_GT pGT, BYTE * pbyStr, HB_SIZE ulLen )
{
  HB_SYMBOL_UNUSED( pGT );
...
static void hb_gt_wvw_OutErr( PHB_GT pGT, BYTE * pbyStr, HB_SIZE ulLen )
{
  HB_SYMBOL_UNUSED( pGT );
...
static BOOL hb_gt_wvw_Info( PHB_GT pGT, int iType, PHB_GT_INFO pInfo )
{
   ...
   switch ( iType )
   {
      ...
      case HB_GTI_CLIPBOARDDATA:
         if( hb_itemType( pInfo->pNewVal ) & HB_IT_STRING )
         {
            hb_gt_w32_setClipboard( pWindowData->CodePage == OEM_CHARSET ?
                                    CF_OEMTEXT : CF_TEXT,
...
//            hb_vmPushNumInt( ( HB_LONG ) ( HB_PTRDIFF ) hWnd );
    ...
    case WM_CHAR:
    case WM_SYSCHAR:
       return hb_gt_wvw_KeyEvent( 	pWindowData, message, wParam, lParam, usWinNum, hWnd );
...
//   pWindowData->cSBColorBackground = NULL;
  ...
  pWindowData->bMaximized   = FALSE ;                   /* Flag is set when window has been maximized */
  pWindowData->bBeingMarked = FALSE ;                 /* Flag to control DOS window like copy operation */
  pWindowData->bBeginMarked = FALSE ;  
...
static void hb_gt_wvwMouseEvent( WIN_DATA * pWindowData, HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam )
{
  ...
  if ( ! pWindowData->bBeginMarked && ! pWindowData->MouseMove  &&( message == WM_MOUSEMOVE || message == WM_NCMOUSEMOVE ) )
    return;
  ...
  switch( message )
  {
    ...
    case WM_LBUTTONDOWN:
      {
         if( pWindowData->bBeginMarked )
         {
            pWindowData->bBeingMarked = TRUE;
    ...
    case WM_LBUTTONUP:
      ...
      //break;
      {
         if( pWindowData->bBeingMarked )
         {
            pWindowData->bBeginMarked = FALSE;
            pWindowData->bBeingMarked = FALSE;

            RedrawWindow( pWindowData->hWnd, NULL, NULL, RDW_INVALIDATE | RDW_UPDATENOW );

            {
               ...
               colrowRC = hb_gt_wvwGetColRowFromXYRect( pWindowData, rect );
               ...
               for( j = 0, irow = top; irow <= bottom; irow++ )
               {
                  for( icol = left; icol <= right; icol++ )
                  {
                     BYTE bColor, bAttr;
                     USHORT usChar;
                     if( !HB_GTSELF_GETSCRCHAR( pWindowData->pGT, irow, icol, &bColor, &bAttr, &usChar ) )
                        break;
    ...
    case WM_MOUSEMOVE:
       {
         if( pWindowData->bBeingMarked )
         {
            ...
            colrowRC = hb_gt_wvwGetColRowFromXYRect( pWindowData, rect );
            rect = hb_gt_wvwGetColRowFromXYRect( pWindowData, colrowRC );
                ...
                rect.bottom != s_rectOld.bottom  )
            {
               ...
               {
                  HDC hdc = GetDC( pWindowData->hWnd );
                  InvertRgn( hdc, rgn3 );
                  ReleaseDC( pWindowData->hWnd, hdc );
               }
...
//    }
    ...
    hb_gt_wvwWindowPrologue( );
    hb_gtInitStatics(s_pWvwData->s_usNumWindows-1, lpszWinName, (USHORT)iRow1, (USHORT)iCol1, (USHORT)iRow2, (USHORT)iCol2,s_pWvwData->pGT);
...
{ /*assume s_pWvwData->s_usNumWindows >= 2 (ie. we are not closing main window)
   ...
   */
    ...
    if ( pWindowData->hWnd )
    {
      ...
      */
      ...
      while (pWindowData->pcdCtrlList)
      {
        ...
        {
           hb_itemRelease( pWindowData->pcdCtrlList->phiCodeBlock );
           pWindowData->pcdCtrlList->phiCodeBlock = NULL;
    ...
    {
      if (!UnregisterClass( s_pWvwData->szSubWinName,( HINSTANCE ) hb_hInstance ))
      {
...
//        hb_vmPushNumInt( ( HB_LONG ) ( HB_PTRDIFF ) s_pWvwData->s_pWindows[ usWinNum ]->hWnd    );
       hb_vmPushPointer( s_pWvwData->s_pWindows[ usWinNum ]->hWnd    );
       hb_vmPushNumInt( message );
       hb_vmPushNumInt( wParam  );
//        hb_vmPushNumInt( lParam  );
       hb_vmPushPointer((void*)   lParam  );
                            ...
                            BYTE * pBuffer )
{
  ...
  if ( s_pWvwData->s_bMainCoordMode )
  {
   ...
   while( i_Top <= i_Bottom )
   {
      ...
      {
         if( !HB_GTSELF_GETCHAR( pGT, i_Top, iCol, &bColor, NULL, &usChar ) )
         {
...
int HB_EXPORT hb_gt_wvwGetLastMenuEvent( UINT usWinNum )
{
...
int HB_EXPORT hb_gt_wvwSetLastMenuEvent( UINT usWinNum, int iLastMenuEvent )
{
...
int HB_EXPORT hb_gt_wvwGetWindowTitle( UINT usWinNum, char *title, int length )
{
...
HWND HB_EXPORT hb_gt_wvwGetWindowHandle( UINT usWinNum )
{
...
void HB_EXPORT hb_gt_wvwPostMessage( UINT usWinNum, int message )
{
...
BOOL HB_EXPORT hb_gt_wvwSetWindowPos( UINT usWinNum, int left, int top )
{
  RECT wi = { 0 };
...
BOOL HB_EXPORT hb_gt_wvwSetAltF4Close( BOOL bCanClose )
{
  BOOL bWas;
...
void HB_EXPORT hb_gt_wvwDoProcessMessages( UINT usWinNum )
{
...
BOOL HB_EXPORT hb_gt_wvwSetMouseMove( UINT usWinNum, BOOL bHandleEvent )
{
...
BOOL HB_EXPORT hb_gt_wvwEnableShortCuts( UINT usWinNum, BOOL bEnable )
{
...
BOOL GetIPictDimension(IPicture * pPic, int * pWidth, int * pHeight)
{
  ...
  pPic->lpVtbl->get_Handle( pPic,(OLE_HANDLE *)&oHtemp );

  GetObject( oHtemp, sizeof(BITMAP), (LPVOID)&bmTemp);
...
BOOL HB_EXPORT hb_gt_wvwRenderPicture( UINT usWinNum, int x1, int y1, int wd, int ht, IPicture * iPicture,
                                       BOOL bTransp )
{
  LONG     lWidth,lHeight;
  int      x,y,xe,ye;
  ...
  if ( iPicture )
  {
    ...
    if ( bTransp )
    {
      ...
      iPicture->lpVtbl->get_Handle( iPicture, (OLE_HANDLE *)&oHtemp );
...
HB_FUNC( WVW_NOPENWINDOW )
{
  ...
  INT   iParentWin = ( ISNIL( 7 ) ? ( s_pWvwData->s_bMainCoordMode ? (INT)s_pWvwData->s_usNumWindows-1 : (INT)s_pWvwData->s_usCurWindow ) : ((INT) hb_parni( 7 )) );
...
{
   UINT usWinNum = WVW_WHICH_WINDOW;
   HB_RETHANDLE( ( HWND ) hb_gt_wvwGetWindowHandle( usWinNum ) ) ;
}
...
// 
//   }
  pcdNew->phiCodeBlock      = phiCodeBlock ? hb_itemNew( phiCodeBlock ) : NULL;
  pcdNew->bBusy = FALSE;
  pcdNew->uiBusy = 0;
                          ...
                          int iStyle )
{
   ...
   if(hWndButton)
   {
     ...
     AddControlHandle(usWinNum, WVW_CONTROL_PUSHBUTTON, hWndButton, uiPBid,  phbiCodeBlock, rXB, rOffXB, (byte) iStyle);
     ...
     HB_STOREHANDLE((HWND)hWndButton,12);
...
// function to treat all key responses in one place
