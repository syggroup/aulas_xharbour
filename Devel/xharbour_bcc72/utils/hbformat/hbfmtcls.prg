/*
 * $Id: hbfmtcls.prg 10058 2014-12-02 12:13:20Z zsaulius $
 */
...
METHOD New( aParams, cIniName ) CLASS HBFORMATCODE
   ...
   IF !( ",STR," $ Upper( ::cFunctions ) )
      ::cFunctions += "iif,ISNIL,ISARRAY,ISBLOCK,ISCHARACTER,ISDATE,ISLOGICAL,ISMEMO,ISNUMBER,ISOBJECT"
...
METHOD ConvertFnc( cLine, nBegin, nEnd ) CLASS HBFORMATCODE
   ...
   IF ::lCase .AND. ::nCaseFnc > 0
      ...
      IF ( nPos := At( "," + cToken + ",", ::cFunctions ) ) != 0
...
STATIC FUNCTION FindNotQuoted( subs, stroka, nPos2 )
   ...
   DO WHILE .T.
      ...
      IF ( nPos1 := At( subs, stroka, nPos2 ) ) == 0
         EXIT
      ENDIF
