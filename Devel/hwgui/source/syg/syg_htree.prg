/*
 * $Id: htree.prg 1789 2011-10-28 02:34:33Z LFBASSO $
 *
...
#define TVN_SELCHANGING      ( - 401 ) // indica que vai mudar
#define TVN_SELCHANGED       ( - 402 ) // indica que mudou
#define TVN_GETDISPINFO      ( - 403 ) // fornecem informações necessárias para exibir ou classificar um item
#define TVN_SETDISPINFO      ( - 404 ) // Notifica janela um controle de árvore-vista do pai que ele deve atualizar as informações que mantém sobre um item
...
#define TVN_KEYDOWN          ( - 412 )  //Notifica janela um controle de árvore-vista do pai que o usuário pressionou uma tecla eo controle de exibição de árvore tem o foco de entrada.
...
CLASS SYG_HTreeNode INHERIT HObject

   DATA handle
   DATA oTree, oParent
   DATA aItems INIT { }
   DATA bAction, bClick, bDblClick
   DATA cargo
   DATA title
   DATA image1, image2
   DATA lchecked INIT .F.

   METHOD New( oTree, oParent, oPrev, oNext, cTitle, bAction, aImages, bDblClick, lchecked, bClick )
   METHOD SYG_AddNode( cTitle, oPrev, oNext, bAction, aImages, bDblClick )
...
METHOD New( oTree, oParent, oPrev, oNext, cTitle, bAction, aImages, bDblClick, lchecked, bClick ) CLASS SYG_HTreeNode
   ...
   IF oPrev == Nil .AND. oNext != Nil
      ...
      ENDIF
   ENDIF
   ::handle := TreeAddNode( Self,;
                            oTree:handle,;
                            IIf( oParent == Nil, Nil, oParent:handle ), ;
                            IIf( oPrev == Nil, Nil, oPrev:handle ),;
                            nPos,;
                            cTitle,;
                            im1,;
                            im2 )
...
METHOD SYG_AddNode( cTitle, oPrev, oNext, bAction, aImages, bDblClick ) CLASS SYG_HTreeNode
   LOCAL oParent := Self
   LOCAL oNode := SYG_HTreeNode():New( ::oTree, oParent, oPrev, oNext, cTitle, bAction, aImages, bDblClick )

   RETURN oNode

METHOD Delete( lInternal ) CLASS SYG_HTreeNode
...
METHOD FindChild( h ) CLASS SYG_HTreeNode
...
METHOD Checked( lChecked ) CLASS SYG_HTreeNode
   LOCAL state
...
METHOD GetLevel( h ) CLASS SYG_HTreeNode
   LOCAL iLevel := 1
...
CLASS SYG_HTree INHERIT HControl
...
CLASS VAR winclass   INIT "SysTreeView32"
   ...
   METHOD SearchString( cText, iNivel, oNode, inodo, lParcial )
   METHOD Selecteds( oItem, aSels )

ENDCLASS

METHOD New( oWndParent, nId, nStyle, nLeft, nTop, nWidth, nHeight, oFont, bInit, bSize, color, bcolor, ;
            aImages, lResour, lEditLabels, bAction, nBC, bRClick, bDblClick, lcheckbox,  bCheck, lDragDrop, bDrag, bDrop, bOther ) CLASS SYG_HTree
   LOCAL i, aBmpSize
...
METHOD Activate() CLASS SYG_HTree
...
METHOD onEvent( msg, wParam, lParam ) CLASS SYG_HTree
   Local nAct, nEval, hitemNew, htiParent, htiPrev, htiNext

*hwg_WriteLog('onEvent: '+str(msg) + str(wParam) + str(loword(wParam))+str(hiword(wParam))+chr(13) )
   ...
   ELSEIF msg == WM_LBUTTONUP .AND. ::lDragging .AND. ::hitemDrop != Nil
      ...
      IF  ! IsCtrlShift( .T. )
         IF ::hitemDrop:oParent != Nil
            hitemNew := ::hitemDrop:oParent:SYG_AddNode( ::hitemDrag:GetText(), htiPrev ,htiNext, ::hitemDrag:bAction ,,::hitemDrag:bDblClick, ::hitemDrag:lchecked, ::hitemDrag:bClick  ) //, ::hitemDrop:aImages )
         ELSE
            hitemNew := ::SYG_AddNode( ::hitemDrag:GetText(), htiPrev ,htiNext, ::hitemDrag:bAction ,, ::hitemDrag:bDblClick, ::hitemDrag:lchecked, ::hitemDrag:bClick  ) //, ::hitemDrop:aImages )
         ENDIF
         DragDropTree( ::hitemDrag, hitemNew , ::hitemDrop ) //htiParent )
      ELSEIF ::hitemDrop != Nil
         hitemNew := ::hitemDrop:SYG_AddNode( ::hitemDrag:Title, htiPrev ,htiNext, ::hitemDrag:bAction,, ::hitemDrag:bDblClick, ::hitemDrag:lchecked, ::hitemDrag:bClick ) //, ::hitemDrop:aImages )
...
METHOD SYG_AddNode( cTitle, oPrev, oNext, bAction, aImages, bDblClick ) CLASS SYG_HTree
   LOCAL oNode := SYG_HTreeNode():New( Self, Nil, oPrev, oNext, cTitle, bAction, aImages, bDblClick )
   ::lEmpty := .F.
   RETURN oNode

METHOD FindChild( h ) CLASS SYG_HTree
   LOCAL aItems := ::aItems, i, alen := Len( aItems ), oNode
   FOR i := 1 TO alen
      IF aItems[ i ]:handle == h
         RETURN aItems[ i ]
...
METHOD FindChildPos( oNode, h ) CLASS SYG_HTree
...
METHOD SearchString( cText, iNivel, oNode, inodo, lParcial ) CLASS SYG_HTree
   LOCAL aItems := IIF( oNode = Nil, ::aItems,  oNode:aItems )
   Local  i:=0 , alen := Len( aItems )
   LOCAL oNodeRet
   IF lParcial=NIL
   ...
   FOR i := 1 TO aLen
      IF ! Empty( aItems[ i ]:aItems ) .AND. ;
         ( oNodeRet := ::SearchString( cText, iNivel, aItems[ i ], iNodo, lParcial ) ) != Nil
         RETURN oNodeRet
      ENDIF
...
METHOD ItemHeight( nHeight )  CLASS SYG_HTree
...
METHOD Notify( lParam )  CLASS SYG_HTree
   ...
   ELSEIF nCode = TVN_KEYDOWN // teclado
*      RETURN 1
      IF ::oItem:oTree:bKeyDown != Nil
         Eval( ::oItem:oTree:bKeyDown, ::oItem, nKeyDown, Self )
      ENDIF

   ELSEIF nCode == NM_DBLCLK .AND. ::oitem != Nil // .AND. !::lEditLabels
       *nHitem :=  Tree_GetNotify( lParam, 1 )

       *IF ! ::lEditLabels .AND. EMPTY( nHitem )
          IF ! ::oItem:oTree:lEmpty
             IF ::oItem:bDblClick != Nil
                Eval( ::oItem:bDblClick, ::oItem, Self )
                RETURN -1
...
METHOD Selecteds( oItem, aSels )  CLASS SYG_HTree
   LOCAL i, iLen
...
STATIC PROCEDURE DragDropTree( oDrag, oItem, oDrop )
   ...
   FOR i := 1 TO iLen
      hitemNew := oItem:SYG_AddNode( oDrag:aItems[ i ]:GetText(), ,, oDrag:aItems[ i ]:bAction,, oDrag:aItems[ i ]:bDblClick, oDrag:aItems[ i ]:lchecked, oDrag:aItems[ i ]:bClick  ) //, ::hitemDrop:aImages )
