...
CLASS VAR winclass   INIT "SysTabControl32"
   ...
   DATA TabHeightSize INIT 22
   DATA internalPaint INIT 0 HIDDEN
   DATA lNAOTraduz      INIT .F.        // Traduzir Itens COMBO ?
...
METHOD StartPage( cname, oDlg, lEnabled, tColor, bColor, cTooltip, lNAOTraduz  ) CLASS HTab

   ::oTemp := ::oDefaultParent
   ::oDefaultParent := Self
   ::lNAOTraduz     := IIf( lNAOTraduz  == Nil, .F., lNAOTraduz  )

   // BEGIN PAGE - Ana Brock - Projeto TRADUZIR - 19/01/2018
   IF ::lNAOTraduz
      cname   := cname
      cTooltip:= cTooltip
   ELSE
...
METHOD OnEvent( msg, wParam, lParam ) CLASS HTab
   ...
   IF msg = WM_LBUTTONDOWN
      ...
      RETURN -1
/*
   ELSEIF  wParam = VK_ESCAPE .and. msg=135 // saindo com esc foi bloqueado
...
*/
   ...
   ELSEIF  msg = WM_GETDLGCODE
      if wParam = VK_ESCAPE // não deixar sai com ESC quando o focu tiver na aba
         RETURN DLGC_WANTMESSAGE
      elseIF wparam == VK_RETURN  .OR. wParam = VK_ESCAPE .AND. ;
