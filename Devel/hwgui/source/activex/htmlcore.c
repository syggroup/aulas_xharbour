/*
 * $Id: htmlcore.c 2364 2015-01-29 10:36:53Z alkresin $
 *
 ...
 *
 * Interfaces used: IOleInPlaceFrame, IOleClientSite, IDocHostUIHandler,
 *                  IOleInPlaceSite, IDispatch.
 *
...
// though we need to provide at least some stub for all of the functions).
//
// So, for these "dummy functions" that we don't expect the browser to call,
// we'll just stick in some assembly code that causes a debugger breakpoint
// and tells the browser object that we don't support the functionality. That
// way, if you try to do more things with the browser object, and it starts
// calling these "dummy functions", you'll know which ones you should add
// more meaningful code to.
#define NOTIMPLEMENTED return(E_NOTIMPL)

/*
 * The following are two functions, SetEmbedded() and GetEmbedded(), which
 * are needed for using with HwGUI.
 * In original code the browser object's pointer was stored in GWL_USERDATA.
...
// by calling our IOleClientSite's QueryInterface (ie, Site_QueryInterface) and
// specifying a REFIID of IID_IDocHostUIHandler.
//
      ...
      REFIID riid, LPVOID * ppvObj )
{
   ...
   // our IDocHostUIHandler's QueryInterface() to ask us to return a pointer
   // to our IOleClientSite, in the same way that the browser calls our
   // IOleClientSite's QueryInterface() to ask for a pointer to our IDocHostUIHandler.
   //
      ...
      DOCHOSTUIINFO * pInfo )
{
   ...
   // lots of other things. See the MSDN docs on the DOCHOSTUIINFO struct passed to us.
...
// object allows us to expose our own automation interface (ie, our own COM
// objects) to other entities that are running within the context of the browser
// so they can call our functions if they want. An example could be a javascript
      ...
      IDispatch ** ppDispatch )
{
   ...
   // NOTE: If we want/need an IDispatch interface, then we would have had to
   // setup our own IDispatch functions, IDispatch VTable, and create an
   // IDispatch object. We'd want to put a pointer to the IDispatch object
   // in our custom _IDocHostUIHandlerEx object (like how we may add an HWND
   // member for the use of UI_ShowContextMenu). So when we defined our
   // _IDocHostUIHandlerEx object, maybe we'd add a 'idispatch' member to the
   // end of it, and store a pointer to our IDispatch object there. Then we
   // could return this pointer as so:
   //
   // *ppDispatch = ((_IDocHostUIHandlerEx FAR *)This)->idispatch;
   // return(S_OK);

   // But for our purposes, we don't need an IDispatch object, so we'll tell
      ...
      DWORD dwTranslate, OLECHAR * pchURLIn, OLECHAR ** ppchURLOut )
{
   ...
   // where XXX is some number. For example, maybe the following will be placed
   // on the web page:
   //
   // <A HREF="app:1">Some special link</A>
   //
   // When the user clicks on it, we will substitute a blank page, and then
   // send the application a WM_APP message with wParam as the number after
      ...
      REFIID riid, void **ppvObject )
{
   ...
   // 'This' is declared is an IOleClientSite *. Remember that in EmbedBrowserObject(),
   // we allocated our own _IOleClientSiteEx struct, and lied to OleCreate()
   // and DoVerb() -- passing our _IOleClientSiteEx struct and saying it was an
   // IOleClientSite struct. It's ok. An _IOleClientSiteEx starts with an
   // embedded IOleClientSite, so the browser didn't mind. So that's what the
   // browser object is passing us now. The browser doesn't know that it's
   // really an _IOleClientSiteEx struct. But we do. So we can recast it and
   // use it as so here.

   // If the browser is asking us to match IID_IOleClientSite, then it wants us
   // to return a pointer to our IOleClientSite struct. Then the browser will
   // use the VTable in that struct to call our IOleClientSite functions. It
   // will also pass this same pointer to all of our IOleClientSite functions.
   //
   // Actually, we're going to lie to the browser again. We're going to return
   // our own _IOleClientSiteEx struct, and tell the browser that it's a
   // IOleClientSite struct. It's ok. The first thing in our _IOleClientSiteEx
   ...
   // Those will always get the pointer to our _IOleClientSiteEx.
   //
   ...
   // will use the VTable in that struct to call our IDocHostUIHandler functions.
   // It will also pass this same pointer to all of our IDocHostUIHandler functions
   // (except for Site_QueryInterface, Site_AddRef, and Site_Release. Those will
   // always get the pointer to our _IOleClientSiteEx.
   //
   // Actually, we're going to lie to the browser. We're going to return our own
   // _IDocHostUIHandlerEx struct, and tell the browser that it's a IDocHostUIHandler
   // struct. It's ok. The first thing in our _IDocHostUIHandlerEx is an embedded
   // IDocHostUIHandler, so the browser doesn't mind. We want the browser to
      ...
      REFIID riid, LPVOID * ppvObj )
{
   ...
   // our IOleClientSite object. So it is possible that the browser may call
   // our IOleInPlaceSite's QueryInterface() to ask us to return a pointer to
   // our IOleClientSite, in the same way that the browser calls our
   // IOleClientSite's QueryInterface() to ask for a pointer to our IOleInPlaceSite.
   //
   // Rather than duplicate much of the code in IOleClientSite's QueryInterface,
   // let's just get a pointer to our _IOleClientSiteEx object, substitute it
   // as the 'This' arg, and call our IOleClientSite's QueryInterface. Note
      ...
      HWND * lphwnd )
{
   ...
   // function declaration for Site_GetWindow says that 'This' is an
   // IOleInPlaceSite *. Remember that in EmbedBrowserObject(), we allocated
   // our own _IOleInPlaceSiteEx struct which contained an embedded
   // IOleInPlaceSite struct within it. And when the browser called
   // Site_QueryInterface() to get a pointer to our IOleInPlaceSite object, we
      ...
      LPOLEINPLACEFRAMEINFO lpFrameInfo )
{
   ...
   // it and use it as so here.
   //
   // Actually, we're giving the browser a pointer to our own
   // _IOleInPlaceSiteEx struct, but telling the browser that it's a
   // IOleInPlaceSite struct. No problem. Our _IOleInPlaceSiteEx starts with
      ...
      DWORD extraData, long id, IUnknown * obj, void *userdata )
{
         ...
         htmlWindow2 )
   {
                        ...
                        varDisp.DEF_VT ) ) != NULL )
      {
         ...
         // Now we attach our IDispatch to the "beforeunload" event so that
      ...
      IHTMLDocument2 ** htmlDoc2Result )
{
   ...
   if( webBrowser2Result || htmlDoc2Result )
   {
      ...
      // Now the pointer to our IWebBrowser2 object is in 'webBrowser2', and
...
void WINAPI UnEmbedBrowserObject( HWND hwnd )
{
   ...
   // window. If 0, you may have called this for a window that wasn't
   // successfully passed to EmbedBrowserObject(). Bad boy! Or, we may have
   // failed the EmbedBrowserObject() call in WM_CREATE, in which case, our
...
#endif
{
   ...
   if( !GetWebPtrs( hwnd, &webBrowser2, 0 ) )
   {
      ...
      // Before we can get the IHTMLDocument2, we actually need to have some
      // HTML page loaded in the browser. So, let's load an empty HTML page.
      // Then, once we have that empty page, we can get that IHTMLDocument2 and
      // call its write() to stuff our HTML string into it.

      // Call the IWebBrowser2 object's get_Document so we can get its DISPATCH
      // object. I don't know why you don't get the DISPATCH object via the
...
#endif
{
   ...
   if( !GetWebPtrs( hwnd, &webBrowser2, 0 ) )
   {
      ...
      // Our URL (ie, web address, such as "http://www.microsoft.com" or an
      // HTM filename on disk such as "c:\myfile.htm") must be passed to the
      // IWebBrowser2's Navigate2() function as a BSTR.
      // A BSTR is like a pascal version of a double-byte character string.
      // In other words, the first unsigned short is a count of how many
      // characters are in the string, and then this is followed by those
      // characters, each expressed as an unsigned short (rather than a char).
      // The string is not nul-terminated. The OS function SysAllocString can
      // allocate and copy a UNICODE C string to a BSTR. Of course, we'll need
      // to free that BSTR after we're done with it. If we're not using UNICODE,
      // we first have to convert to a UNICODE string.
      //
      // What's more, our BSTR needs to be stuffed into a VARIENT struct, and
      // that VARIENT struct is then passed to Navigate2(). Why? The VARIENT
      // struct makes it possible to define generic 'datatypes' that can be
      // used with all languages. Not all languages support things like
...
void WINAPI ResizeBrowser( HWND hwnd, DWORD width, DWORD height )
{
   ...
   if( !GetWebPtrs( hwnd, &webBrowser2, 0 ) )
   {
      ...
      // that we'll still maintain our hold on the browser object.
      webBrowser2->lpVtbl->Release( webBrowser2 );
   }
}
...
long WINAPI EmbedBrowserObject( HWND hwnd )
{
   ...
   // Our IOleClientSite, IOleInPlaceSite, and IOleInPlaceFrame functions need
   // to get our window handle. We could store that in some global. But then,
   // that would mean that our functions would work with only that one window.
   // If we want to create multiple windows, each hosting its own browser
   // object (to display its own web page), then we need to create unique
   // IOleClientSite, IOleInPlaceSite, and IOleInPlaceFrame structs for each
   // window. And we'll put an extra member at the end of those structs to
   // store our extra data such as a window handle. So, our functions won't
   // have to touch global data, and can therefore be re-entrant and work with
   // multiple objects/windows.
   //
   // Remember that a pointer to our IOleClientSite we create here will be
   // passed as the first arg to every one of our IOleClientSite functions.
   // Ditto with the IOleInPlaceFrame object we create here, and the
   // IOleInPlaceFrame functions. So, our functions are able to retrieve the
   // window handle we'll store here, and then, they'll work with all such
   // windows containing a browser control.
   //
   // Furthermore, since the browser will be calling our IOleClientSite's
   // QueryInterface to get a pointer to our IOleInPlaceSite and
   // IDocHostUIHandler objects, that means that our IOleClientSite
   // QueryInterface must have an easy way to grab those pointers. Probably
   // the easiest thing to do is just embed our IOleInPlaceSite and
   // IDocHostUIHandler objects inside of an extended IOleClientSite which
   // we'll call a _IOleClientSiteEx. As long as they come after the pointer
   // to the IOleClientSite VTable, then we're ok.
   //
   // Of course, we need to GlobalAlloc the above structs now. We'll just get
   // all 3 with a single call to GlobalAlloc, especially since they're are
   // contained inside of our _IOleClientSiteEx anyway.
   //
   // So, we're not actually allocating separate IOleClientSite,
   // IOleInPlaceSite, IOleInPlaceFrame and IDocHostUIHandler structs.
   //
   ...
   // "disappear" while we're using it in this program). We do this by calling
   // the OS function CoCreateInstance().
   //
   // NOTE: We need this pointer to interact with and control the browser.
   // With normal WIN32 controls such as a Static, Edit, Combobox, etc, you
   //  obtain an HWND and send messages to it with SendMessage(). Not so with
   // the browser object. You need to get a pointer to it. This object contains
   // an array of pointers to functions you can call within the browser object.
   // Actually, it contains a 'lpVtbl' member that is a pointer to that
   // array. We call the array a 'VTable'.
   //
   // For example, the browser object happens to have a SetClientSite()
   // function we want to call. So, after we retrieve the pointer to the
   // browser object (in a local we'll name 'browserObject'), then we can call
   // that function, and pass it args, as so:
   //
   // browserObject->lpVtbl->SetClientSite(browserObject, SomeIOleClientObject);
   //
   // There's our pointer to the browser object in 'browserObject'. And
   // there's the pointer to the browser object's VTable in
   // 'browserObject->lpVtbl'. And the pointer to the SetClientSite function
   // happens to be stored in a member named 'SetClientSite' within the VTable.
   // So we are actually indirectly calling SetClientSite by using
               ...
               &IID_IWebBrowser2, ( void ** ) &webBrowser2 ) )
   {
      ...
      // above, and then save the pointer to that whole thing in our window's
      // USERDATA member. That way, if we need multiple windows each hosting
      ...
      if( ( *( ( IOleObject ** ) ptr ) = browserObject ) != NULL )
      {
         ...
         //
         // NOTE: We pass our _IOleClientSiteEx struct and lie -- saying that
