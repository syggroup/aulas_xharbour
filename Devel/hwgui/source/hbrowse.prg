...
CLASS HBrowse INHERIT HControl
   ...
   DATA lNAOTraduz      INIT .F.        // Traduzir Header ?

   METHOD New( lType,oWndParent,nId,nStyle,nLeft,nTop,nWidth,nHeight,oFont, ;
               bInit,bSize,bPaint,bEnter,bGfocus,bLfocus,lNoVScroll,;
               lNoBorder,lAppend,lAutoedit,bUpdate,bKeyDown,bPosChg,lMultiSelect,;
               lDescend, bWhile, bFirst, bLast, bFor, bOther,tcolor, bcolor, brclick, bChgRowCol, ctooltip, lNAOTraduz )
...
METHOD New( lType, oWndParent, nId, nStyle, nLeft, nTop, nWidth, nHeight, oFont, ;
            bInit, bSize, bPaint, bEnter, bGfocus, bLfocus, lNoVScroll, ;
            lNoBorder, lAppend, lAutoedit, bUpdate, bKeyDown, bPosChg, lMultiSelect, ;
            lDescend, bWhile, bFirst, bLast, bFor, bOther, tcolor, bcolor, bRclick, bChgRowCol, ctooltip, lNAOTraduz ) CLASS HBrowse

//   lNoVScroll := IIf( lNoVScroll = Nil , .f., lNoVScroll ) // leonardo machado - 25-06-2014
   ...
   // BROWSE / TOOLTIP - Ana Brock - Projeto TRADUZIR - 30/10/2019
   If ::lNAOTraduz
      ctooltip := ctooltip
   Else
      ...
      IF !oColumn:lHide
       IF ::lDispHead .AND. ! ::lAppMode
         ...
         FOR nLine := 1 TO ::nHeadRows
            ...
            // BROWSE - Ana Brock - Projeto TRADUZIR - 27/01/2018
            If ::lNAOTraduz
               cStr := cStr
            Else
...
METHOD ButtonDown( lParam, lReturnRowCol ) CLASS HBrowse
   ...
   CATCH
      lReturnRowCol:=.T.
   END
   //fif := aColumns[ IIF(fif>LEN(aColumns),1,fif), 2 ]
...
//   IF  ( ! Eval( ::bEof, Self ) .OR. ::lAppMode ) .AND. ( ! ::onClick( )  )
   IF  ( ! Eval( ::bEof, Self ) .OR. ::lAppMode ) .AND. fipos > 0 .AND. ;
      ...
      IF ::lEditable .AND. Type != "O" .AND. ( oColumn:aList != Nil .OR.  ( oColumn:aList = Nil .AND. wParam != 13 ) )
         ...
         ELSE
            INIT DIALOG oModDlg CLIPPER NOEXIT TITLE "Editar Texto" At 0, 0 SIZE 400, 300;
         ...
         IF oColumn:aList != Nil  .AND. ( oColumn:bWhen = Nil .OR. Eval( oColumn:bWhen ) )
            ...
            @ 0, 0 GET COMBOBOX oCombo VAR nChoic ;
               ...
               FONT oComboFont  ;
               ON CHANGE {|| KEYB_EVENT( VK_RETURN ),.T. };
         ...
         ELSE
            ...
            ELSEIF Type <> "M"
               ...
               @ 0, nHGet GET oGet VAR ::varbuf       ;
                  ...
                  TOOLTIP 'Informe o conteudo do campo'
            ...
            ELSE
               ...
               @ 010, 252 BUTTONEX "Salvar"              size 80, 32 ON Click { || ::varbuf := MaskBinData(oGet1), oModDlg:close(), oModDlg:lResult := .t. }
               @ 100, 252 BUTTONEX "Fechar"              size 80, 32 ON CLICK { || oModDlg:close() }
               @ 210, 252 BUTTONEX "Salvar em Arquivo"   size 180,32 ON Click { || MemoWrit( 'CAMPO_MEMO.TXT', UnMaskBinData(ALLTRIM(::varbuf)) ),Hwg_MsgInfo('Arquivo (CAMPO_MEMO.TXT) salvo com Sucesso','Aviso do Sistema')}
            ENDIF
         ENDIF
...
//showmsg({numf,Len( oBrw:aColumns ) ,oBrw:aColumns[ numf ]:length })
...
LOCAL aRET:={}
aRET:= { | value, o, n | IIf( value == Nil, o:aArray[ IIf( o:nCurrent < 1, 1, o:nCurrent ), n ],;
